...
examples-jit: elle
	@echo "=== examples (JIT enabled, threshold=0, release) ==="
	@export ELLE_JIT_THRESHOLD=0 &&printf '%s\n' examples/*.lisp | \
		grep -v allocator.lisp | \
		parallel -j $(JOBS) --halt now,fail=1 --tag \
			'timeout $(TIMEOUT) ./target/release/elle {}' \
		|| { echo "FAILED: examples JIT pass"; exit 1; }
