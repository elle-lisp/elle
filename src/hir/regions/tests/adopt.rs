...
fn owned_subtree_upvalue_capture_owner_refused_on_lifetime() {
    ...
    // `e` ALSO captures `m` (the forward every upvalue implies). External uniqueness
    // ADMITS the subtree {o, e, m} with `o` as root, and the refusal MUST hold: `o`'s
    // region is minted per CALL of `e`, so adopting `m`/`e` (which survive across calls)
    // would free them under `e`'s still-live references and re-adopt an Owned region on
    // the next call. The lifetime obligation refuses structurally — the forwarding
    // capture resolves `m`'s tight last-use through `e`'s binding chain to a position
    // at/past the enclosing lambda node, which post-dates `o`'s in-body drop in
    // post-order — and for THIS letrec shape the merge-overlap filter independently
    // refuses first (the enclosing `e`'s self-edge + compiled cell make it a
    // closure-cycle-merge participant), so the family is out even if either gate is
    // loosened alone. Three halves:
    ...
    //   (2) the obligation's own refusal CAUSE is pinned: `m`'s tight last-use
    //       over-extends past the root's decref_point (the exact condition it refuses
    //       on), independent of the merge overlap;
    //   (3) no adopt is emitted — the family stays Shared (the always-legal baseline)
    //       until an owner that outlives every capturer (the activation/fiber node)
    //       exists.
    ...
    let owner = owned
        ...
        .map(|(r, _)| *r)
        .expect("compute_owned_subtrees must admit the externally-unique {o, e, m} subtree");
    assert!(
    ...
    // (2) the refusal cause: the tight last-use (the obligation bound for a
    // capture-suppressed member) over-extends past the root's drop. A change that makes
    // this pass — a tighter transitive last-use, or a root whose drop moves out of the
    // enclosing body — flips this precondition and forces the author to re-derive the
    // soundness argument (the per-call-root double-adopt) before admitting the family.
    let order = compute_order(&hir);
    let ord = |id: HirId| order.get(&id).copied().unwrap_or(0);
    let root_dp = ord(info
        .region_data
    ...
    assert!(
        ...
        "precondition (the refusal cause): the upvalue-captured member r{}'s tight \
         ...
         ({root_dp}); if it no longer does, the lifetime obligation would admit a \
         per-call root owning a cross-call member — re-derive the soundness argument \
         before flipping the assertion below",
        m.0,
        owner.0,
    );
