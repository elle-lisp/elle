...
#[test]
fn fileless_error_string_parses_to_the_unknown_file_sentinel() {
    // "line:col: message" with no file part defaults the file to the same
    // sentinel SourceLoc uses for unknown origin, so a parsed-back location
    // is recognised as unknown rather than treated as a file literally
    // named "<unknown>".
    let (file, line, col, msg) = parse_located_error("12:5: oops").unwrap();
    assert_eq!(file, crate::reader::UNKNOWN_FILE);
    assert!(SourceLoc::new(file, line, col).is_unknown());
    assert_eq!((line, col, msg), (12, 5, "oops"));
}
