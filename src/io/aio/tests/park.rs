...
//! The last two are the exception: each holds both backends to one answer.
//!
//! What becomes of an operation whose FIBER went, rather than one a caller
//! cancelled, is in `gone.rs`; what becomes of its descriptor is in
//! `descriptor.rs`.
...
fn a_cancelled_pool_process_wait_ends_rather_than_being_abandoned() {
    crate::value::arena::with_test_region(|| {
        ...
        let child = std::process::Command::new("sleep")
            ...
            .expect("spawn sleep");
        let pid = child.id();
        let handle = h.ctx().external(SUBPROCESS, ProcessHandle::new(pid, child));
