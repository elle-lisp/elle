...
/// `drain_cqes` → `PendingOp::SigNext` → `parse_events`. The test
/// drives that helper itself, so a change that routed the ring's
...
fn sig_next_uring_child_logic() -> i32 {
    ...
    // submit_sig_next in aio/externals.rs allocates.
