...
pub(crate) fn prim_list_primitives(
    ...
    (
        SIG_QUERY,
        ctx.pair(Value::keyword("list-primitives"), filter),
    )
}
...
pub(crate) fn prim_primitive_meta(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    (
        SIG_QUERY,
        ctx.pair(Value::keyword("primitive-meta"), args[0]),
    )
}

/// (fn/disasm closure) — disassemble bytecode as array of strings
pub(crate) fn prim_disbit(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    if let Some(closure) = args[0].as_closure() {
        ...
        (
            SIG_OK,
            ctx.array_mut(lines.into_iter().map(|s| ctx.string(s)).collect()),
        )
    } else {
        (
            SIG_ERROR,
...
pub(crate) fn prim_fn_flow(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    if let Some(closure) = args[0].as_closure() {
        flow_from_closure(ctx, closure)
    } else if let Some(handle) = args[0].as_fiber() {
        match handle.try_with(|fiber| flow_from_closure(ctx, &fiber.closure)) {
            Some(result) => result,
            None => (
                SIG_ERROR,
                ctx.error(
