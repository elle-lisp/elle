...
pub use crate::primitives::defmeta::{Doc, PrimitiveMeta};
...
pub struct PrimitiveDef {
    ...
    /// Capability bits this call requires that depend on its ARGUMENTS, not on
    /// the declaration in `signal`. Returns the extra bits to add to the
    /// capability gate's requirement for a given argument list.
    ///
    /// `io/submit` is the declarant: it spends whatever operation its request
    /// argument carries, so its requirement is that request's own
    /// [`crate::io::request::IoOp::required_bits`] — a spawn request makes the
    /// submit require `:exec`, a plain read requires only `:io`. This is the
    /// argument-dependent analogue of a `Signal`'s `propagates` mask for a
    /// closure parameter: a native whose authority is carried by a value rather
    /// than fixed by its name. `None` (the default) for every primitive whose
    /// requirement is exactly its declared `signal.bits`.
    pub bits_from_args: Option<fn(&[Value]) -> crate::value::fiber::SignalBits>,
}
