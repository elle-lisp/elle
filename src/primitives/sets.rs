...
/// - @bytes → bytes
pub(crate) fn freeze_value(v: Value, ctx: &mut crate::primitives::ctx::Alloc) -> Value {
    if let Some(arr) = v.as_array_mut() {
        let items: Vec<Value> = arr.borrow().iter().map(|x| freeze_value(*x, ctx)).collect();
        ctx.array(items)
        ...
        let items: BTreeSet<Value> = s.borrow().iter().map(|x| freeze_value(*x, ctx)).collect();
        ctx.set(items)
    } else if let Some(buf) = v.as_string_mut() {
        let bytes = buf.borrow().clone();
        ctx.string(&*String::from_utf8_lossy(&bytes))
...
pub(crate) fn prim_set(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    let mut set = BTreeSet::new();
    for arg in args {
        set.insert(freeze_value(*arg, ctx));
    }
    (SIG_OK, ctx.set(set))
}
...
pub(crate) fn prim_contains(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    match coll_has(&args[0], &args[1], ctx) {
...
pub(crate) fn prim_union(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    match coll_combine(&args[0], &args[1], ctx) {
        Ok(v) => (SIG_OK, v),
        Err(e) => (SIG_ERROR, e),
    }
}
...
pub(crate) fn prim_intersection(
    ctx: &mut crate::primitives::ctx::NativeCtx<'_>,
    args: &[Value],
) -> (SignalBits, Value) {
    ...
    match (set_elements(&args[0]), set_elements(&args[1])) {
        (Some(sa), Some(sb)) => {
            ...
            if is_mutable(&args[0]) {
                (SIG_OK, ctx.set_mut(result))
            } else {
                (SIG_OK, ctx.set(result))
            }
        }
        _ => (
            SIG_ERROR,
...
pub(crate) fn prim_seq_to_set(
    ...
    if v.is_empty_list() {
        return (SIG_OK, ctx.set(BTreeSet::new()));
    }
    if v.as_pair().is_some() {
        ...
        loop {
            ...
            if let Some(pair) = current.as_pair() {
                set.insert(freeze_value(pair.first, ctx));
                current = pair.rest;
            } else {
                return (
                    SIG_ERROR,
        ...
        return (SIG_OK, ctx.set(set));
    }

    // Array (immutable) → immutable set
    if let Some(elems) = v.as_array() {
        let set: BTreeSet<Value> = elems.iter().map(|x| freeze_value(*x, ctx)).collect();
        return (SIG_OK, ctx.set(set));
    }
    ...
    if v.is_string() {
        ...
        v.with_string(|s| {
            for ch in s.chars() {
                set.insert(ctx.string(ch.to_string()));
            }
        });
        return (SIG_OK, ctx.set(set));
    }
    ...
    if let Some(arr) = v.as_array_mut() {
        let set: BTreeSet<Value> = arr.borrow().iter().map(|x| freeze_value(*x, ctx)).collect();
        return (SIG_OK, ctx.set_mut(set));
    }
