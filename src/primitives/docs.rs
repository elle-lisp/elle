...
pub(crate) fn register_builtin_docs(docs: &mut std::collections::HashMap<String, Doc>) {
    ...
    let builtins: &[Doc] = &[
        ...
        Doc {
            name: "environment",
            doc: "Reify the current lexical scope as a struct. Returns a struct mapping quoted symbols to their values for all lexical bindings in scope. Primitives are excluded. Useful with eval to pass the current environment.",
            params: &[],
            arity: Arity::Exact(0),
            signal: Signal::silent(),
            category: "special form",
