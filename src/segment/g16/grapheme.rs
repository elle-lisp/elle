...
use super::tables::grapheme::GraphemeCat;
...
impl GraphemeCursor {
    ...
    fn grapheme_category(&mut self, ch: char) -> GraphemeCat {
        use super::tables::grapheme as gr;
        use super::tables::grapheme::GraphemeCat::*;
    ...
    fn handle_incb_consonant(&mut self, chunk: &str, chunk_start: usize) {
        use super::tables::{self, grapheme as gr};
    ...
    pub fn next_boundary(
        ...
        loop {
            if self.resuming {
                ...
                if super::tables::is_incb_linker(ch) {
                    self.incb_linker_count = Some(self.incb_linker_count.map_or(1, |c| c + 1));
                } else if !super::tables::derived_property::InCB_Extend(ch) {
        ...
        loop {
            ...
            if self.resuming {
                ...
                if let Some(incb_linker_count) = self.incb_linker_count {
                    // Upstream 1.12.0 wrote this countdown into `ris_count`,
                    // breaking reverse iteration over InCB conjuncts (GB9c).
                    // This is the fix that shipped in 1.13.x, applied to the
                    // correct field; pinned by segment::tests::double_ended_matches_forward.
                    self.incb_linker_count =
                        if incb_linker_count > 0 && super::tables::is_incb_linker(ch) {
