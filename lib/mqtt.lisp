...
(fn [plugin]
  ...
  (defn send-packet [conn packet-bytes]
    ...
    (when (nonempty? packet-bytes) (port/write conn:tcp packet-bytes)))

  ## ── Public API ────────────────────────────────────────────────────

  (defn
    mqtt/connect
    [host port-num &named client-id username password @clean-session keep-alive]
    "Connect to an MQTT broker. Returns {:tcp port :mqtt mqtt-state}."
    (default clean-session true)
    (let* [opts {:client-id client-id
                 :username username
                 :password password
  ...
  (defn mqtt/publish [conn topic payload &named @qos retain]
    ...
    (let [opts {:qos qos :retain retain}]
      ...
      (when (>= qos 1) (drive-until conn (packet-type? :puback)))))
  ...
  (defn mqtt/listen [conn callback]
    ...
    (forever (if-let [pkt (mqtt/recv conn)] (callback pkt) (break))))
