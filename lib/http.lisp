...
(fn [&named tls compress]
    ...
    (block :chunks
      (forever
        (let [size (chunk-size (t-read-line t))]
          (when (= size 0)  # Consume optional trailers until the blank line.
            (forever
              (let [line (t-read-line t)]
          ...
          (while (pos? remaining)
            (let [chunk (t-read t remaining)]
              ...
              (let [b (if (bytes? chunk) chunk (bytes chunk))]
                (append buf b)
                (assign remaining (- remaining (length b))))))  # Consume the CRLF that terminates the chunk data.
          (t-read-line t))))
    (string (freeze buf)))
  ...
  (defn with-file-transport [path mode thunk]
    ...
    (let [p (port/open path mode)]
      (defer (protect (port/close p)) (thunk (tcp-transport p)))))
