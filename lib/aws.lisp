...
(def creds
  {:access-key (sys/env "AWS_ACCESS_KEY_ID")
   :secret-key (sys/env "AWS_SECRET_ACCESS_KEY")
   :session-token (sys/env "AWS_SESSION_TOKEN")})
(def default-region
  (or (sys/env "AWS_DEFAULT_REGION") (sys/env "AWS_REGION") "us-east-1"))
...
(defn decode-body [body headers]
  (if (nil? body)
    ...
    (let* [ct (or (get headers :content-type) "")
           json? (or (string-contains? ct "json")
             (string-contains? ct "x-amz-json"))
           text? (or (string-contains? ct "text/") (string-contains? ct "xml"))]
      (cond
...
(defn read-response [tls conn raw]  ## Read status
  (let* [line (tls:read-line conn)
    ...
    (forever
      (let [hline (tls:read-line conn)]
        (when (or (nil? hline) (= (string/trim hline) "")) (break nil))
        (let [colon (string/find hline ":")]
          (when (not (nil? colon))
            (put resp-headers (keyword (string/lowercase (slice hline 0 colon)))
              (string/trim (slice hline (inc colon))))))))  ## Read body
    (let* [te (get resp-headers :transfer-encoding)
           cl (get resp-headers :content-length)
           resp-body (cond
                       (and te
                         (string-contains? (string/lowercase te) "chunked"))
                         (block :chunked
                           (def chunks @[])
                           (forever
                             (let* [sz-line (tls:read-line conn)
                               ...
                               (when (= sz 0)
                                 (tls:read-line conn)
                                 (break :chunked (if (empty? chunks)
                                   (bytes)
                                   (apply concat chunks))))
                               (let [chunk (tls:read conn sz)]
                                 (push chunks chunk)
                                 (tls:read-line conn)))))
                       (not (nil? cl)) (tls:read conn (parse-int cl))
                       true nil)
...
(defn aws-request-impl [tls service method path opts]
  (let* [region (or (get opts :region) default-region)
         ...
         headers (if (nil? body)
                   headers
                   (merge headers
                     {:content-length (string (if (string? body)
                        (string/size-of body)
                        (length body)))}))]
