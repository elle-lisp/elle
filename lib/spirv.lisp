...
(defn emit-entry-point [section exec-model fn-id name interface-ids]
  ...
  (let* [str-words (string-to-words name)
    ...
    (push section fn-id)
    (each w str-words (push section w))
    (each id interface-ids (push section id))))
...
(defn serialize [m bound]
  (let [buf (@bytes)]
    ...
    (encode-word buf 0)
    (each w (m :caps)        (encode-word buf w))
    (each w (m :mem-model)   (encode-word buf w))
    (each w (m :entry)       (encode-word buf w))
    (each w (m :exec-mode)   (encode-word buf w))
    (each w (m :decorations) (encode-word buf w))
    (each w (m :types)       (encode-word buf w))
    (each w (m :functions)   (encode-word buf w))
    buf))
...
(defn compute [local-size-x num-buffers body-fn f32-bits]
  ...
  (let* [m   (make-module)
    ...
    ## ── Execution mode ──────────────────────────────
    (emit-inst (m :exec-mode) op-execution-mode
      main-fn exec-mode-local-size local-size-x 1 1)
