...
(defn zmq/term [ctx]
  ...
  (check (zmq-ctx-term ctx) "zmq/term")
  nil)
...
(defn zmq/close [sock]
  (check (zmq-close sock) "zmq/close")
  nil)
(defn zmq/bind [sock ep]
  (check (zmq-bind sock ep) "zmq/bind")
  nil)
(defn zmq/connect [sock ep]
  (check (zmq-connect sock ep) "zmq/connect")
  nil)
(defn zmq/unbind [sock ep]
  (check (zmq-unbind sock ep) "zmq/unbind")
  nil)
(defn zmq/disconnect [sock ep]
  (check (zmq-disconnect sock ep) "zmq/disconnect")
  nil)
...
(defn zmq/send-multipart [sock frames]
  ...
  (let [last-idx (- (length frames) 1)]
    (each i in (range (length frames))
      (zmq/send sock (get frames i) :sndmore (< i last-idx)))))
...
(defn zmq/recv-multipart [sock &named dontwait]
  ...
  (let [parts @[]]
    ...
    (while (zmq/has-more? sock) (push parts (zmq/recv sock :dontwait dontwait)))
    (freeze parts)))
