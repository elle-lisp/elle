...
(fn []
  ...
  (defn strip-padding [s]
    ...
    (def @end (length s))
    (while (and (> end 0) (= (s (dec end)) "=")) (assign end (dec end)))
    (slice s 0 end))
  ...
  (defn encode-url [data]
    "Base64-encode (URL-safe, no pad)."
    (encode-with data url-chars false))
