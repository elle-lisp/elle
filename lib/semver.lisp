...
(fn []
  (defn parse-int-strict [s ctx]
    ...
    (when (empty? s)
      (error {:error :semver-error
              :message (string ctx ": empty version component")}))
  ...
  (defn compare-pre [a b]
    ...
    (if (= a b)
      0
      (if (= a "")
        1  # no pre-release > any pre-release
        (if (= b "")
          -1
          (let [as (string/split a ".")
                bs (string/split b ".")]
            (defn cmp-ids [ai bi]
              (if (and (>= ai (length as)) (>= bi (length bs)))
                0
                (if (>= ai (length as))
                  -1
                  (if (>= bi (length bs))
  ...
  (defn parse-op [s]
    ...
    (let [len (length s)]
      (if (string/starts-with? s ">=")
        [(string/trim (slice s 2 len)) ">="]
        (if (string/starts-with? s "<=")
          [(string/trim (slice s 2 len)) "<="]
          (if (string/starts-with? s "!=")
            [(string/trim (slice s 2 len)) "!="]
            (if (string/starts-with? s ">")
              [(string/trim (slice s 1 len)) ">"]
              (if (string/starts-with? s "<")
                [(string/trim (slice s 1 len)) "<"]
                (if (string/starts-with? s "=")
                  [(string/trim (slice s 1 len)) "="]
    ...
    (let* [trimmed (string/trim req-str)
      ...
      (match op
        ...
        "~"
          (let [rv (parse ver-str)
                pv (parse ver)]
            (and (>= c 0) (= pv:major rv:major) (= pv:minor rv:minor)))
        _
          (error {:error :semver-error
