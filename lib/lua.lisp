            ...
            (if (= t :boolean)
              ...
              (if (= t :nil)
                ...
                (if (= t :closure)
                  ...
                  (if (= t :native-fn)
                    "function"
                    (if (or (= t :array) (= t :@array))
                      "table"
                      (if (or (= t :struct) (= t :@struct)) "table" "userdata"))))))))))))
...
(def lua_print (fn (& args) (println (string/join (map string args) "\t"))))
...
(def string_lib
  {:len length
   ...
   :rep (fn (s n) (string/join (map (fn (_) s) (range n)) ""))
   :format string
   :byte (fn (s i) (get (bytes s) (if (nil? i) 0 (- i 1))))
...
(def table
  {:insert (fn (t & args)
             (if (= (length args) 1) (push t (get args 0)) (push t (get args 1))))
   :remove (fn (t & args) (pop t))
   :concat (fn (t sep) (string/join (map string t) (if (nil? sep) "" sep)))
   :unpack (fn (t) t)})
...
(def pcall
  (fn (f & args)
    (let [[ok result] (protect ((fn () (apply f args))))]
      (if ok [true result] [false result]))))

# lua_error: raise an error (can't use `error` — already a builtin)
(def lua_error
  (fn (msg)
    (if (string? msg) (error {:error :lua-error :message msg}) (error msg))))
...
(def select
  (fn (index & args) (if (= index "#") (length args) (get args (- index 1)))))
