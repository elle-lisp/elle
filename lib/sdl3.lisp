...
(fn []
  ...
  (def fcolor-type (ffi/struct @[:float :float :float :float]))  # SDL_Vertex = SDL_FPoint position + SDL_FColor color + SDL_FPoint tex_coord
  (def vertex-type
  ...
  (defn sdl/poll-events []
   ...
   Each event has at minimum :type and :timestamp."
    (let [events @[]]
      (while (sdl-poll-event event-buf) (push events (marshal-event event-buf)))
      events))
   ...
   :channels defaults to 2 (stereo). :freq defaults to 48000."
    (ffi/with-stack [[spec audio-spec-type
                    ...
                    (check-ptr (sdl-open-audio-device-stream (if device
                                 device
                                 audio-device-default-playback) spec nil nil)
                               "sdl/open-audio")))
