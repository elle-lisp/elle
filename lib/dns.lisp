...
(defn decode-name [buf offset]
  ...
  {:name (string/join (freeze parts) ".") :offset (or return-offset (+ pos 1))})
...
(defn format-ipv4 [buf offset]
  ...
  (string/join (list (string (get buf offset))
      ...
      (let [record (case rtype
                     TYPE-A
                       (begin
                         ...
                         {:type :a
                          :name name
                          :addr (format-ipv4 buf rdata-start)
                          :ttl ttl})
                     TYPE-AAAA
                       (begin
                         (when (not (= rdlen 16))
                         ...
                         {:type :aaaa
                          :name name
                          :addr (format-ipv6 buf rdata-start)
                          :ttl ttl})
                     TYPE-CNAME
                       (let [cname-result (decode-name buf rdata-start)]
                         {:type :cname
                          :name name
                          :target cname-result:name
                          :ttl ttl})
                     nil)]
               ...
               (unless (= resp:header:rcode RCODE-OK)
                 (let [rcode-name (or (get rcode-names resp:header:rcode)
                   ...
                   (error {:error :dns-error
                           ...
                           :name name
                           :server server
                           :message (concat "server returned "
                           rcode-name
                           " for "
                           name)})))
               resp)))))
           ...
           direct (filter (fn [r]
                            (= r:type
                               (case qtype
                                 ...
                                 TYPE-AAAA :aaaa
                                 nil)))
                          answers)  # Check for CNAME redirects
...
(defn resolve [name &named server timeout retries]
  ...
  (let* [srv (or server (first (read-nameservers)))
         ...
         a-records (let [[ok? result] (protect (resolve-type name
                         TYPE-A
                         srv
                         tmo
                         ret))]
                     (if ok? result ()))
         aaaa-records (let [[ok? result] (protect (resolve-type name
                            TYPE-AAAA
                            srv
                            tmo
                            ret))]
...
(defn run-internal-tests []
  ...
  (let* [txid 0x5678
    ...
    (let [a (first resp:answers)]
      ...
      (assert (= a:addr "2606:2800:220:1:248:1893:25c8:1946") "aaaa parse: addr")))
  ...
  (let* [txid 0x9999
         ...
         cname-answer (concat (bytes 0xc0 12)  # Name pointer
                              ...
                              cname-target)  # A answer for the CNAME target
