...
(fn []
  ...
  (def z-deflateInit2 (cfn libz "deflateInit2_" :int @[:ptr :int :int :int :int :int :string :int]))
  ...
  (def zstd-compress   (cfn libzstd "ZSTD_compress"            :size @[:ptr :size :ptr :size :int]))
  (def zstd-decompress (cfn libzstd "ZSTD_decompress"          :size @[:ptr :size :ptr :size]))
  ...
  (def zstd-frame-size (cfn libzstd "ZSTD_getFrameContentSize" :i64 @[:ptr :size]))
  ...
  (defn deflate-compress [data window-bits level]
    ...
    (let* [input (bytes data)
      ...
      (let [rc (z-deflateInit2 stream level 8 window-bits 8 0 ZLIB_VERSION Z_STREAM_SIZE)]
        (unless (= rc Z_OK)
          (ffi/free stream)
          (ffi/free out-buf)
          (error {:error :compress-error :message (string "deflateInit2 failed: " rc)})))
      ## Set input
      ...
      ## Actually total_out offset: next_out(24,8) + avail_out(32,4) + pad(36,4) + total_out(40,8)
  ...
  (defn deflate-decompress [data window-bits]
    ...
    (let* [input (bytes data)
      ...
      (let [rc (z-inflateInit2 stream window-bits ZLIB_VERSION Z_STREAM_SIZE)]
        (unless (= rc Z_OK)
          (ffi/free stream)
          (error {:error :compress-error :message (string "inflateInit2 failed: " rc)})))
      ## Set input
      ...
      (while (not done)
        (let [space (- buf-size total-out)]
          ...
          (let [rc (z-inflate stream 0)]  # Z_NO_FLUSH = 0
              ...
              0 (when (zero? (ffi/read (ptr/add stream 32) :u32))
                   ...
                   (let* [new-size (* buf-size 2)
                     ...
                     (each i in (range total-out)
                       (ffi/write (ptr/add new-buf i) :u8
  ...
  (defn gzip    [data & opts] (deflate-compress data 31  (if (> (length opts) 0) (first opts) 6)))
  (defn gunzip  [data]        (deflate-decompress data 31))
  (defn zlib    [data & opts] (deflate-compress data 15  (if (> (length opts) 0) (first opts) 6)))
  (defn unzlib  [data]        (deflate-decompress data 15))
  (defn deflate [data & opts] (deflate-compress data -15 (if (> (length opts) 0) (first opts) 6)))
  (defn inflate [data]        (deflate-decompress data -15))
  ...
  (defn unzstd [data]
    (let* [input (bytes data)
      ...
      (when (< frame-size 0)
        (error {:error :compress-error :message "unzstd: cannot determine frame size"}))
      (if (zero? frame-size) (bytes)
  ...
  {:gzip gzip :gunzip gunzip
   :zlib zlib :unzlib unzlib
   :deflate deflate :inflate inflate
   :zstd zstd :unzstd unzstd})
