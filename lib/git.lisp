...
(fn []
  ...
  (def c-index-remove-bypath (cfn "git_index_remove_bypath" :int @[:ptr :string]))
  (def c-index-add-all (cfn "git_index_add_all"       :int @[:ptr :ptr :int :ptr :ptr]))
  (def c-index-update-all (cfn "git_index_update_all" :int @[:ptr :ptr :ptr :ptr]))
  ...
  (def c-commit-create (cfn "git_commit_create"       :int @[:ptr :ptr :string :ptr :ptr :ptr :string :ptr :size :ptr]))
  ...
  (def c-branch-iterator-new (cfn "git_branch_iterator_new" :int @[:ptr :ptr :int]))
  ...
  (def c-branch-create (cfn "git_branch_create"       :int @[:ptr :ptr :string :ptr :int]))
  ...
  (def c-tag-create-lightweight (cfn "git_tag_create_lightweight" :int @[:ptr :ptr :string :ptr :int]))
  (def c-tag-create  (cfn "git_tag_create"            :int @[:ptr :ptr :string :ptr :ptr :string :int]))
  ...
  (def c-config-get-string (cfn "git_config_get_string" :int @[:ptr :ptr :string]))
  (def c-config-set-string (cfn "git_config_set_string" :int @[:ptr :string :string]))
  ...
  (def c-set-head-detached (cfn "git_repository_set_head_detached" :int @[:ptr :ptr]))
  ...
  (defn repo-path [repo] (ffi/string (c-repo-path repo)))
  (defn workdir [repo] (maybe-str (c-repo-workdir repo)))
  (defn bare? [repo] (not (zero? (c-repo-bare repo))))
    ...
    (let* [slist (with-pp (fn [pp] (check (c-status-list-new pp repo null-ptr) "git/status")))
      ...
      (each i in (range count)
        (let* [entry (c-status-byindex slist i)
               ...
               ## git_diff_delta: status(4) + flags(4) + similarity(2) + nfiles(2) + old_file(?) + new_file(?)
               ## git_diff_file: oid(20 bytes) + path(ptr,8) + size(i64,8) + flags(u32,4) + mode(u16,2) + id_abbrev(u16,2) = 44 → padded 48
               ## So old_file.path is at delta+8+20=delta+28? No: delta starts with status(u32,4)+flags(u32,4)+similarity(u16,2)+nfiles(u16,2)=12
          ...
          (when (not (= path-delta null-ptr))
            ...
            (push results {:path "" :index (status-keyword flags true) :workdir (status-keyword flags false)}))))
  ...
  {:open open :init init :clone clone-repo :close close
   :path repo-path :workdir workdir :bare? bare? :state state
   :head head :resolve resolve
   :commit-info commit-info :log log :commit commit
   :status status :add add :remove remove
   :branches branches :branch-create branch-create :branch-delete branch-delete
   :tags tags :tag-create tag-create :tag-delete tag-delete
   :remotes remotes :remote-info remote-info :fetch fetch
