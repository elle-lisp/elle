  ...
  (defn log [repo & opts]
    (let* [opt (if (> (length opts) 0) (first opts) {})
      ...
      (let* [oid-buf (ffi/malloc GIT_OID_SIZE)
        ...
        (ffi/free oid-buf)
        (->list results))))
  (defn commit [repo message & opts]  ## Get index and write tree
    ...
    (let* [slist (with-pp (fn [pp]
      ...
      (each i in (range count)
        (let* [entry (c-status-byindex slist i)
               ...
               ## Actually easier: just read from
               ## head_to_index or index_to_workdir delta

               h2i (ffi/read (ptr/add entry 8) :ptr)

               ## delta->new_file.path is at offset 96
               ## on x86_64 (after old_file which is 88 bytes)

               i2w (ffi/read (ptr/add entry 16) :ptr)

               ## git_diff_file layout:
               ##   oid(20) + path_ptr(8) + size(8) +
               ##   flags(4) + mode(2) + id_abbrev(2) = 44
               ##   padded to 48
               ##
               ## git_diff_delta layout:
               ##   status(4) + flags(4) + similarity(2) +
               ##   nfiles(2) + old_file(?) + new_file(?)
               ##
               ## old_file starts at 16 (aligned).
               ## old_file.oid is 20 bytes, path is at
               ## old_file+24 (after oid+padding).
               ##
               ## oid is { unsigned char id[20]; } so
               ## old_file.path is at offset
               ## 16+20+4(pad)=40? This is fragile.
               ##
               ## Better: just get the path from whichever
               ## delta is non-null.
