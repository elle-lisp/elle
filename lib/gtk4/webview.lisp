...
(fn []
  ...
  (defn make-webview (win-handle props)
    ...
    (let* [ptr (b:webkit-web-view-new)
      ...
      (w:make-widget win-handle
                     (merge props {:hexpand true :vexpand true})
                     ptr
                     :webview)))
  ...
  (defn webview-eval (win-handle id js)
    ...
    (when-let [wgt (win-handle:widgets id)]
              (b:webkit-web-view-evaluate-javascript wgt:ptr
                js
                -1
                nil
                nil
  ...
  (defn webview-send (win-handle id msg)
    ...
    (webview-eval win-handle
                  id
                  (string "if(window.elle&&window.elle"
                          ".onMessage)window.elle"
                          ".onMessage("
                          (json-escape msg)
                          ")")))
