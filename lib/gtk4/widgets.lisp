...
(fn []
  ...
  (defn register-widget (win-handle id ptr type)
    ...
    (when id (put win-handle:widgets id @{:ptr ptr :type type})))
  ...
  (defn make-text-edit (win-handle props)
    (let [ptr (b:gtk-text-view-new)]
      ...
      (when props:rows (b:gtk-widget-set-size-request ptr -1 (* props:rows 20)))
  ...
  (defn make-scroll-area (win-handle props)
    (let [ptr (b:gtk-scrolled-window-new)]
      (when props:height
        (b:gtk-scrolled-window-set-min-content-height ptr props:height))
      (when props:width
        (b:gtk-scrolled-window-set-min-content-width ptr props:width))
  ...
  (defn make-grid (win-handle props)
    (let [ptr (b:gtk-grid-new)]
      ...
      (when props:col-spacing
        (b:gtk-grid-set-column-spacing ptr props:col-spacing))
  ...
  (defn make-drawing-area (win-handle props)
    (let [ptr (b:gtk-drawing-area-new)]
      ...
      (when props:height
        (b:gtk-drawing-area-set-content-height ptr props:height))
      (when props:on-draw
  ...
  (defn add-scroll (win-handle ptr handler)
    ...
    (let* [scroll (b:gtk-event-controller-scroll-new SCROLL_VERTICAL)
           cb (ffi/callback sig-scroll (fn (ctrl dx dy data) (handler dx dy)))]
      (push win-handle:callbacks cb)
  ...
  {:null? null?
   ...
   :make-widget make-widget  # display
   :make-label make-label
   :make-heading make-heading
   :make-image make-image
   :make-progress-bar make-progress-bar
   :make-separator make-separator
   :make-spacer make-spacer
   :make-spinner make-spinner  # input
   :make-button make-button
   :make-toggle-button make-toggle-button
   :make-text-input make-text-input
   :make-text-edit make-text-edit
   :make-checkbox make-checkbox
   :make-switch make-switch
   :make-slider make-slider
   :make-spin-button make-spin-button
   :make-combo-box make-combo-box
   :make-search-entry make-search-entry
   :make-calendar make-calendar  # layout
   :make-box make-box
   :make-scroll-area make-scroll-area
   :make-expander make-expander
   :make-frame make-frame
   :make-grid make-grid
   :make-stack make-stack
   :make-notebook make-notebook
   :make-paned make-paned
   :make-center-box make-center-box
   :make-overlay make-overlay
   :make-revealer make-revealer  # drawing
   :make-drawing-area make-drawing-area  # event controllers
   :add-click add-click
