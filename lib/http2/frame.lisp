...
(fn []
  ...
  (defn run-tests []  # ── Byte helpers ──
    ...
    (let* [write-buf @[]
           mock-transport {:write (fn [data]
                                    (let [b (if (string? data) (bytes data) data)]
                                      (push write-buf b)))
                           :read nil}]
      (write-frame mock-transport TYPE-HEADERS 0x5 1 (bytes 0x82 0x86))  # Reconstruct what was written
