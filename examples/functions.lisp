...
(println "  (average 80 90) = " (average 80 90))
...
(println "  (letter-grade 95) = " (letter-grade 95))
...
(println "  (doc letter-grade) = " (doc letter-grade))
## ── Lexical scope ──────────────────────────────────────────────────
...
(let ([midterm 82]
      [final 94])
  (assert (= (average midterm final) 88) "let: temporary bindings for grade calc"))
...
(let* ([raw 78]
  ...
  (assert (= grade "B") "let*: sequential — curved then graded"))
...
(letrec ([sum-scores (fn [lst]
  ...
  (assert (= (sum-scores (list 90 80 70)) 240) "letrec: recursive sum"))
...
(let ([passing 70])                # shadows the outer 'passing'
  (assert (= passing 70) "shadowing: inner passing"))
(assert (= passing 60) "shadowing: outer passing unchanged")
## ── Closures ───────────────────────────────────────────────────────
...
(assert (= (grader 95) "A") "grader: first score")
(assert (= (grader 72) "C") "grader: second score")
...
(assert (= (my-map letter-grade scores) (list "C" "B" "A" "D" "A")) "my-map: letter grades")
...
(assert (= (my-filter passing? scores) (list 72 85 90 68 95)) "my-filter: all pass here")
(assert (= (my-filter (fn [s] (>= s 80)) scores) (list 85 90 95)) "my-filter: B or above")
...
(println "  sum of scores = " (my-fold + 0 scores))
...
(println "  (curved-grade 78) = " (curved-grade 78))
...
(assert (= (->> scores
       ...
       (my-map letter-grade)) (list "B" "A" "A")) "->>: filter then map")
...
(def class-avg
  (let* ([passing-scores (my-filter passing? scores)]  # keep ≥60
    ...
    (/ total count)))                                  # divide
(assert (= class-avg 82) "pipeline: class average")
## ── Variadic functions ─────────────────────────────────────────────
...
(assert (= (grade-all 95 82 71) (list "A" "B" "C")) "variadic: grade three scores")
(assert (= (grade-all) (list)) "variadic: no scores")
...
(println "  (best-of 72 85 90 68) = " (best-of 72 85 90 68))
## ── Mutual recursion ───────────────────────────────────────────────
...
(letrec ([expect-pass (fn [lst]
  ...
  (assert (expect-pass (list 80 50 90 40)) "mutual: alternating pass/fail")
  (assert (not (expect-pass (list 80 90 50))) "mutual: not alternating"))
## ── block and break ────────────────────────────────────────────────
...
(assert (= first-failing 68) "block: found first failing score")
...
(def result
  (block :outer
    ...
    "never reached"))
(assert (= result "escaped") "block: break crosses inner block")
## ── Mutable captures and destructuring ─────────────────────────────
...
(println "  running avg after 90,80,70 = " (avg))
...
(assert (= (bonus-points 5) 5) "accumulator: +5")
(assert (= (bonus-points 3) 8) "accumulator: +3")
(assert (= (bonus-points 2) 10) "accumulator: +2")
...
(println "  grade-student({Bob, 87}) →" rname rgrade)
