...
`EllError` exists in `src/error/types.rs` but is unused. The codebase uses:
```rust
...
pub type NativeFn = fn(&[Value]) -> Result<Value, EllError>;
pub type VmAwareFn = fn(&[Value], &mut VM) -> Result<Value, EllError>;
```

### 2.3 Implementation Steps

#### Step 2.3.1: Expand `EllError`

Add `location` and `stack_trace` fields to existing `EllError`.
...
// After
pub fn prim_add(args: &[Value]) -> Result<Value, EllError> {
    if args.len() < 2 {
        return Err(EllError::arity_at_least(2, args.len()));
    }
    // ...
}
```
...
// src/vm/mod.rs
impl VM {
    pub fn execute(&mut self, bytecode: &[u8]) -> Result<Value, EllError> {
...
| `src/vm/mod.rs` | Return EllError, capture stack traces |
| `src/compiler/*.rs` | Return EllError from compilation |
...
- `Err(format!("expected X, got {}", y))` → `Err(EllError::type_mismatch("X", y))`
- `Err("division by zero")` → `Err(EllError::division_by_zero())`
...
impl Syntax {
    ...
    pub fn from_value(value: &Value, symbols: &SymbolTable) -> Result<Syntax, EllError> { ... }
}
```
...
impl Parser {
    pub fn parse(&mut self) -> Result<Syntax, EllError> {
    ...
    fn parse_kind(&mut self) -> Result<SyntaxKind, EllError> {
        match self.peek()? {
...
impl Analyzer {
    ...
    fn resolve_var(&mut self, sym: SymbolId) -> Result<BindingId, EllError> {
        ...
        Err(EllError::undefined(&symbols.name(sym)))
    }
}
```
...
```rust
// During HIR analysis
fn analyze_lambda(&mut self, items: &[Syntax], span: Span) -> Result<HirExpr, EllError> {
    // ...
...
// src/vm/mod.rs
fn handle_invoke_restart(&mut self) -> Result<(), EllError> {
    ...
    Err(EllError::runtime("No matching restart found"))
}
```
...
impl Analyzer {
    fn check_effect_compatibility(&self, caller: Effect, callee: Effect) -> Result<(), EllError> {
        if !caller.permits(&callee) {
            return Err(EllError::effect_violation(caller, callee));
        }
        Ok(())
    }
}
...
// src/compiler/hir/analyze.rs
fn analyze_import(&mut self, module_name: SymbolId) -> Result<(), EllError> {
