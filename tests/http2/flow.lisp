               ...
               (forever
                 (let [[ok? f] (protect (frame:read-frame t 262144))]
                   ...
                   (cond
                     ...
                     (= f:type C:type-headers)
                      (begin
                        # Respond, then send GOAWAY
                        (let* [resp-hdr (hpack:encode enc [[":status" "200"]])
                               [ft fl si pl] (frame:make-headers-frame
                                               f:stream-id resp-hdr true true)]
                          (frame:write-frame t ft fl si pl))
                        (let [[ft fl si pl]
                              (frame:make-goaway-frame f:stream-id C:err-no-error)]
                          (frame:write-frame t ft fl si pl))
                        (t:flush))
                     true nil)))
               (protect (t:close)))))]
    (let [session (http2:connect (concat "http://127.0.0.1:" (string lport)))]
      (defer (begin (protect (http2:close session)) (protect (ev/join sf)))
        (let [resp (http2:send session "GET" "/test")]
          (assert (= resp:status 200) "goaway: first request ok"))
        # Give reader time to process the GOAWAY
        (ev/sleep 0.1)
                      ...
                      (begin
                        (let* [resp-hdr (hpack:encode enc [[":status" "200"]])
                               [ft fl si pl] (frame:make-headers-frame
                                               f:stream-id resp-hdr false true)]
                          (frame:write-frame t ft fl si pl))
                        (let* [body (bytes (concat "resp-" (string f:stream-id)))
                               [ft fl si pl] (frame:make-data-frame
                                               f:stream-id body true)]
                          (frame:write-frame t ft fl si pl))
                        (t:flush))
...
(defn test-settings-window-adjustment-e2e []
  ...
  (let* [[listener lport] (listen-ephemeral)
           ...
           (fn []
             (let [t (make-raw-transport (tcp/accept listener))]
               ...
               (let [payload (concat (frame:u16->bytes C:settings-initial-window-size)
                     ...
                     [ft fl si pl] (frame:make-settings-frame
                                     [[C:settings-initial-window-size 32768]])]
                 (frame:write-frame t ft fl si pl))
               (t:flush)
    ...
    (let [session (http2:connect (concat "http://127.0.0.1:" (string lport)))]
      (defer (begin (protect (http2:close session)) (protect (ev/join sf)))
        # Poll until SETTINGS applied (up to 3s)
        (let [@attempts 0]
          (while (and (not (= (get session:remote-settings :initial-window-size) 32768))
                      (< attempts 30))
            (ev/sleep 0.1)
            (assign attempts (+ attempts 1))))
        (assert (= (get session:remote-settings :initial-window-size) 32768)
                (concat "settings: window adjusted to 32768, got "
                        (string (get session:remote-settings :initial-window-size))))
        # Verify we can still send
