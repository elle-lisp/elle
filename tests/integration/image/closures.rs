...
use elle::value::{Arity, CaptureMask, TemplateProto, TemplateRef};
use elle::SourceLoc;
...
// A template carrying child blueprints fails the dump by name: its
// `MakeClosure` instructions would have nothing to build on the other side.
#[test]
fn a_template_with_child_blueprints_refuses_the_dump() {
    let dir = crate::common::ScratchDir::new("image-closure-children");
    let path = dir.join("children.image");

    let mut src = FiberHeap::new();
    let region = src.new_runtime_region();
    let mut proto = TemplateProto::new(vec![1], Arity::Exact(0), Vec::new());
    proto.name = Some("parent-of-lambdas".to_string());
    proto.child_protos = vec![Rc::new(TemplateProto::new(
        vec![2],
        Arity::Exact(0),
        Vec::new(),
    ))];
    let root = closure_in(&mut src, region, &Rc::new(proto), &[], SignalBits::EMPTY);
    refused(&mut src, root, &path, "parent-of-lambdas");
}
...
fn a_compiled_closure_answers_a_call_after_hydration() {
    ...
    let (signal, arity) = {
        ...
        assert!(
            closure.template.lir_function().is_none(),
            "a hydrated closure runs the interpreter tier"
        );
        (closure.template.signal(), closure.template.arity())
    };
    let sym = rt2.symbols().intern("hydrated-f");
    {
        let (cctx, heap) = rt2.compile_and_heap();
