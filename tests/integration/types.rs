...
#[test]
fn test_type() {
    assert!(eval("(type-of 42)").unwrap().as_keyword().is_some());
    assert!(eval("(type-of 3.14)").unwrap().as_keyword().is_some());
    assert!(eval("(type-of \"hello\")").unwrap().as_keyword().is_some());
}
...
fn test_mixed_type_operations() {
    ...
    if let Some(f) = eval("(+ 1 1.5)").unwrap().as_float() {
