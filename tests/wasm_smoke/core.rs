...
fn test_dump_closure_let_lir() {
    let mut core = elle::runtime::RuntimeCore::bare();
...
// --- Self-recursion via LoadSelf (all-tier self-reference resolution) ---
//
// A reference to a lambda's own self-recursive binding resolves to the executing
// closure through `LoadSelf`, which the WASM backend reads from the reserved self
// slot the host installs at each closure entry. These use only `%`-primitives and
// core forms so `eval_wasm` (no stdlib) compiles them standalone; the result is
// correct only if LoadSelf named the executing closure on WASM.

#[test]
fn test_self_recursion_call_position() {
    // `(go (%sub m 1))` is a call-position self-reference (non-tail, inside %add).
    assert_eq!(
        eval("(letrec [go (fn [m] (if (%lt m 1) 0 (%add 1 (go (%sub m 1)))))] (go 5))"),
        "5"
    );
}

#[test]
fn test_self_recursion_tail_position() {
    // Accumulating tail self-recursion: `(go (%sub m 1) (%add acc m))` is a
    // call-position self-reference in tail position (WASM `return_call_indirect`).
    assert_eq!(
        eval("(letrec [go (fn [m acc] (if (%lt m 1) acc (go (%sub m 1) (%add acc m))))] (go 5 0))"),
        "15"
    );
}

#[test]
fn test_self_recursion_value_position() {
    // `go` in value position (truthiness) must materialize the executing closure,
    // which is truthy → returns m.
    assert_eq!(
        eval("(letrec [go (fn [m] (if go m (%sub 0 1)))] (go 7))"),
        "7"
    );
}
...
fn test_tail_call_accumulator() {
    assert_eq!(
        eval("(letrec [sum (fn [n acc] (if (= n 0) acc (sum (- n 1) (+ acc n))))] (sum 10000 0))"),
        "50005000"
    );
}
