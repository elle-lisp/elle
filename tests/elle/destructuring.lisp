...
(let [[ok? _] (protect ((fn () (def (a b c) (list 1)))))] (assert (not ok?) "def list short: error on missing element"))

# test_def_list_empty_source — empty source signals error
(let [[ok? _] (protect ((fn () (def (a b) (list)))))] (assert (not ok?) "def list empty: error on empty list"))
...
(let [[ok? _] (protect ((fn () (def (a b) 42))))] (assert (not ok?) "def list wrong type: error"))
...
(let [[ok? _] (protect ((fn () (def [x y z] [10]))))] (assert (not ok?) "def array short: error on missing element"))

# test_def_array_wrong_type — wrong type signals error
(let [[ok? _] (protect ((fn () (def [a b] 42))))] (assert (not ok?) "def array wrong type: error"))
...
(assert (= (let [(a b) (list 10 20)] (+ a b)) 30) "let list destructure")

# test_let_array_destructure
(assert (= (let [[x y] [3 4]] (+ x y)) 7) "let array destructure")

# test_let_mixed_bindings
(assert (= (let [a 1 (b c) (list 2 3)] (+ a b c)) 6) "let mixed bindings")

# test_let_nested_destructure
(assert (= (let [((a b) c) (list (list 1 2) 3)] (+ a b c)) 6) "let nested destructure")
...
(assert (= (let* [(a b) (list 1 2) c (+ a b)] c) 3) "let* destructure basic")

# test_let_star_destructure_sequential_reference
(assert (= (let* [(a b) (list 1 2) (c d) (list a b)] (+ c d)) 3) "let* destructure sequential reference")

# test_let_star_mixed_simple_and_destructure
(assert (= (let* [x 10 (a b) (list x 20)] (+ a b)) 30) "let* mixed simple and destructure")

# test_let_star_shadowing_with_destructure
(assert (= (let* [a 1 (a b) (list 10 20)] a) 10) "let* shadowing with destructure")
...
(assert (= (let [(a b) (list 10 20)] ((fn () (+ a b)))) 30) "let destructure in closure")
...
(assert (= (let [(_ b) (list 10 20)] b) 20) "wildcard in let")
...
(assert (= (let [(a & r) (list 10 20 30)] (+ a (first r))) 30) "rest list in let")

# test_rest_list_in_fn_param
(assert (= ((fn ((a & r)) (+ a (first r))) (list 10 20)) 30) "rest list in fn param")
...
(assert (= (let [[a & r] [10 20 30]] (+ a (get r 0))) 30) "rest array in let")
...
(assert (= ((fn (a b & rest) (+ a b)) 10 20 30 40) 30) "variadic fn fixed and rest")
...
(begin
  ...
  (let [[ok? _] (protect ((fn () (eval '(begin (defn f5 (x & rest) x) (f5))))))] (assert (not ok?) "variadic compile time arity: 0 args fails")))
...
(let [[ok? _] (protect ((fn () (def {:missing m} {:other 42}))))] (assert (not ok?) "def struct missing key: error"))

# test_def_struct_wrong_type — wrong type signals error
(let [[ok? _] (protect ((fn () (def {:x x} 42))))] (assert (not ok?) "def struct wrong type: error"))

# test_struct_missing_key_errors (was: test_struct_missing_key_is_nil)
(let [[ok? _] (protect ((fn () (def {:missing m2} {:other 42}))))] (assert (not ok?) "struct missing key errors"))

# test_struct_wrong_type_errors (was: test_struct_wrong_type_is_nil)
(let [[ok? _] (protect ((fn () (def {:x x2} 42))))] (assert (not ok?) "struct wrong type errors"))
...
# test_let_struct
(assert (= (let [{:x x :y y} {:x 10 :y 20}] (+ x y)) 30) "let struct")

# test_let_star_struct
(assert (= (let* [{:x x} {:x 5} {:y y} {:y x}] (+ x y)) 10) "let* struct")
...
(let [[ok? _] (protect ((fn () (let [[a b] 42] a))))] (assert (not ok?) "destructure non-sequential int: error"))
(let [[ok? _] (protect ((fn () (let [[a b] "hello"] a))))] (assert (not ok?) "destructure non-sequential string: error"))
...
# test_opt_multiple
(assert (= ((fn (a &opt b c) (list a b c)) 1) (list 1 nil nil)) "opt multiple: none provided")
(assert (= ((fn (a &opt b c) (list a b c)) 1 2) (list 1 2 nil)) "opt multiple: one provided")
(assert (= ((fn (a &opt b c) (list a b c)) 1 2 3) (list 1 2 3)) "opt multiple: all provided")

# test_opt_too_many_args
(let [[ok? _] (protect ((fn () (eval '((fn (a &opt b) a) 1 2 3)))))] (assert (not ok?) "opt too many args"))

# test_opt_too_few_args
(let [[ok? _] (protect ((fn () (eval '((fn (a &opt b c) a))))))] (assert (not ok?) "opt too few args"))

# test_opt_with_rest
(assert (= ((fn (a &opt b & rest) (list a b rest)) 1) (list 1 nil ())) "opt with rest: none")
(assert (= ((fn (a &opt b & rest) (list a b rest)) 1 2) (list 1 2 ())) "opt with rest: opt only")
(assert (= ((fn (a &opt b & rest) (list a b rest)) 1 2 3 4) (list 1 2 (list 3 4))) "opt with rest: opt and rest")
...
(let [[ok? _] (protect ((fn () (eval '(begin (defn f8 (a &opt b) a) (f8))))))] (assert (not ok?) "opt compile time arity: too few"))
(let [[ok? _] (protect ((fn () (eval '(begin (defn f9 (a &opt b) a) (f9 1 2 3))))))] (assert (not ok?) "opt compile time arity: too many"))

# test_opt_no_params_after
(let [[ok? _] (protect ((fn () (eval '(fn (&opt) 1)))))] (assert (not ok?) "opt no params after &opt"))

# test_opt_after_rest_error
(let [[ok? _] (protect ((fn () (eval '(fn (a & rest &opt b) 1)))))] (assert (not ok?) "opt after rest error"))
...
(assert (= ((fn (a &keys {:x x :y y}) (+ x y)) 1 :x 10 :y 20) 30) "keys destructure")

# test_keys_missing_key_destructure — missing key now signals an error
(let [[ok? _] (protect ((fn () ((fn (a &keys {:x x :y y}) y) 1 :x 10))))] (assert (not ok?) "keys missing key destructure signals error"))

# test_keys_all_present — all keys provided, still works
(assert (= ((fn (a &keys {:x x :y y}) (+ x y)) 1 :x 10 :y 20) 30) "keys destructure all present")

# test_keys_with_opt
(assert (= ((fn (a &opt b &keys opts) (list a b opts)) 1) (list 1 nil {})) "keys with opt: none")
(assert (= ((fn (a &opt b &keys opts) (list a b opts)) 1 2) (list 1 2 {})) "keys with opt: opt only")
(assert (= ((fn (a &opt b &keys opts) (list a b opts)) 1 2 :x 10) (list 1 2 {:x 10})) "keys with opt: opt and keys")

# test_keys_odd_args_error
(let [[ok? _] (protect ((fn () ((fn (a &keys opts) opts) 1 :x 10 :y))))] (assert (not ok?) "keys odd args error"))

# test_keys_non_keyword_key_error
(let [[ok? _] (protect ((fn () ((fn (a &keys opts) opts) 1 42 10))))] (assert (not ok?) "keys non-keyword key error"))

# test_keys_and_rest_exclusive
(let [[ok? _] (protect ((fn () (eval '(fn (a &keys opts & rest) 1)))))] (assert (not ok?) "keys and rest exclusive: keys then rest"))
(let [[ok? _] (protect ((fn () (eval '(fn (a & rest &keys opts) 1)))))] (assert (not ok?) "keys and rest exclusive: rest then keys"))
...
# test_named_basic
(assert (= ((fn (a &named host port) (list host port)) 1 :host "db" :port 3306) (list "db" 3306)) "named basic")

# test_named_missing_key
(assert (= ((fn (a &named host port) port) 1 :host "db") nil) "named missing key")

# test_named_unknown_key_error
(let [[ok? _] (protect ((fn () ((fn (a &named host) host) 1 :host "db" :port 3306))))] (assert (not ok?) "named unknown key error"))

# test_named_with_opt
(assert (= ((fn (a &opt b &named host) (list a b host)) 1 :host "db") (list 1 nil "db")) "named with opt")
...
(let [[ok? _] (protect ((fn () ((fn (a &named host) host) 1 :host))))] (assert (not ok?) "named odd args error"))

# test_named_and_keys_exclusive
(let [[ok? _] (protect ((fn () (eval '(fn (a &keys opts &named host) 1)))))] (assert (not ok?) "named and keys exclusive: keys then named"))
(let [[ok? _] (protect ((fn () (eval '(fn (a &named host &keys opts) 1)))))] (assert (not ok?) "named and keys exclusive: named then keys"))

# test_named_no_params_error
(let [[ok? _] (protect ((fn () (eval '(fn (a &named) 1)))))] (assert (not ok?) "named no params error"))

# test_named_non_symbol_error
(let [[ok? _] (protect ((fn () (eval '(fn (a &named [x]) 1)))))] (assert (not ok?) "named non-symbol error"))
...
(assert (= ((fn (a &opt (b c)) (list a b c)) (list 1 2)) (list (list 1 2) nil nil)) "opt destructuring pattern: not provided")
(assert (= ((fn (a &opt (b c)) (list a b c)) 1 (list 2 3)) (list 1 2 3)) "opt destructuring pattern: provided")
...
(let [[ok? _] (protect ((fn ()
    (begin
      ...
      (defn g () (f11 1 :x))
      (g)))))] (assert (not ok?) "keys tail call error"))
...
(assert (= (let [{'a v} (struct 'a 42)] v) 42) "let struct symbol key")
...
(let [[ok? _] (protect ((fn () (def {'missing m} (struct 'other 1)))))] (assert (not ok?) "symbol key missing: error"))
...
(assert (= (letrec [(a b) (list 1 2)] (+ a b)) 3) "letrec list destructure")

# test_letrec_struct_destructure
(assert (= (letrec [{:x x} {:x 42}] x) 42) "letrec struct destructure")

# test_letrec_array_destructure
(assert (= (letrec [[a b] [10 20]] (+ a b)) 30) "letrec array destructure")
...
(let [[ok? _] (protect ((fn () (def {:missing m} {:other 42}))))] (assert (not ok?) "struct missing key errors (property)"))

# def_struct_non_struct_errors (was: def_struct_non_struct_is_nil)
(let [[ok? _] (protect ((fn () (def {:a a} 42))))] (assert (not ok?) "struct non-struct errors (property)"))

# fn_param_struct_equiv_get
(begin
  (defn f-destr ({:a a :b b}) (+ a b))
  (defn g-manual (t) (+ (get t :a) (get t :b)))
  (assert (= (f-destr {:a 10 :b 20}) (g-manual {:a 10 :b 20})) "fn param struct equiv get"))
...
# let_struct_destr
(assert (= (let [{:a a :b b} {:a 3 :b 7}] (+ a b)) 10) "let struct destructure (property)")

# let_star_struct_forward_ref
(assert (= (let* [{:x v} {:x 5} {:y w} {:y v}] (+ v w)) 10) "let* struct forward ref (property)")
...
(let [[ok? _] (protect ((fn () (eval '(begin (def (a b) (list 1 2)) (assign a 10))))))] (assert (not ok?) "def destructured bindings are immutable"))

# variadic_arity_check_too_few
(let [[ok? _] (protect ((fn () (eval '((fn (a b & rest) a) 1)))))] (assert (not ok?) "variadic arity check: too few args"))

# keys_duplicate_keys
(let [[ok? _] (protect ((fn () ((fn (a &keys opts) (get opts :x)) 1 :x 10 :x 20))))] (assert (not ok?) "duplicate keyword keys error"))
...
(assert (= (let [{:a a & rest} {:a 10}]
    [a rest]) [10 {}]) "struct rest: all keys explicit → rest is empty struct")

# test_struct_rest_in_fn
(assert (= ((fn ({:x x & rest}) rest) {:x 1 :y 2 :z 3}) {:y 2 :z 3}) "struct rest in fn param")

# test_struct_rest_result_is_immutable
(assert (= (type-of (let [{:a a & rest} {:a 1 :b 2}] rest)) :struct) "struct rest result is always immutable struct")

# test_struct_rest_from_mutable
(assert (= (let [{:a a & rest} @{:a 1 :b 2 :c 3}]
    rest) {:b 2 :c 3}) "struct rest from @struct input yields immutable rest")
...
(assert (= ((fn (a &keys {:x x & rest}) rest) 1 :x 10) {}) "keys destructure with rest, no extra keys")

# test_keys_destructure_missing_required_with_rest
(let [[ok? _] (protect ((fn () ((fn (a &keys {:x x & rest}) rest) 1 :y 20))))] (assert (not ok?) "keys destructure missing required key signals error even with rest"))