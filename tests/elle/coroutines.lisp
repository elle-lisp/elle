...
(begin
  ...
  (coro/resume co5)
  (assert (= (string (coro/status co5)) "paused") "state: after second yield is paused")
  (coro/resume co5)
  (assert (= (string (coro/status co5)) "dead") "state: after final return is dead"))
...
(begin
  ...
  (coro/resume co5b)
  (assert (= (string (coro/status co5b)) "dead") "state single: after return is dead"))
...
(begin
  ...
  (assert (= (coro/resume co 20) 44) "multiple yields intermediate: (1+10+3)+(4+20+6)=44"))
...
(begin
  ...
  (assert (= (string (coro/status co)) "paused") "tail position: paused after second yield")
  (coro/resume co)
  (assert (= (string (coro/status co)) "dead") "tail position: dead after final resume"))
