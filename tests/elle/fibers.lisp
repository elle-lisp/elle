...
(let ([f (fiber/new |:yield| (fn [] (emit 2 1) (emit 2 2) 3))])
...
(let ([f (fiber/new |:yield| (fn [] (emit 2 -50) (emit 2 0) (emit 2 50) 999))])
...
(let ([f (fiber/new || (fn [] (emit 1 99)))])
  (let ([wrapper (fiber/new |:error| (fn [] (fiber/resume f)))])
...
(let ([f (fiber/new |:error :yield| (fn [] (emit 2 77)))])
  (assert (= (fiber/resume f) 77) "signal mask: yield caught by mask=3"))
(let ([f (fiber/new |:error :yield| (fn [] (emit 1 88)))])
...
(let ([f (fiber/new |:error :yield| (fn [] (emit 2 0) 99))])
  (fiber/resume f)
...
(let (([ok? _] (protect ((fn []
  (let ([f (fiber/new |:error| (fn [] (emit 1 -50)))])
    (fiber/resume f)
...
(let ([inner (fiber/new |:yield| (fn [] (emit 2 10)))])
  (let ([outer (fiber/new || (fn [] (+ (fiber/resume inner) 5)))])
    (assert (= (fiber/resume outer) 15) "nested resume: 10 + 5 = 15")))

(let ([inner (fiber/new |:yield| (fn [] (emit 2 -30)))])
  (let ([outer (fiber/new || (fn [] (+ (fiber/resume inner) 20)))])
...
(let ([c (fiber/new |:yield| (fn [] (emit 2 10)))])
  (let ([b (fiber/new || (fn [] (+ (fiber/resume c) 5)))])
    (let ([a (fiber/new || (fn [] (+ (fiber/resume b) 3)))])
      (assert (= (fiber/resume a) 18) "3-level nested: 10 + 5 + 3 = 18"))))

(let ([c (fiber/new |:yield| (fn [] (emit 2 -20)))])
  (let ([b (fiber/new || (fn [] (+ (fiber/resume c) 30)))])
    (let ([a (fiber/new || (fn [] (+ (fiber/resume b) -5)))])
...
(begin
  (let ((inner (fiber/new |:yield| (fn [] (emit 2 99)))))
    (let ((outer (fiber/new |:yield| (fn []
                     (fiber/resume inner)
...
(begin
  (let ((f (fiber/new |:error :yield| (fn [] (emit 2 "waiting") 99))))
    (fiber/resume f)
...
(begin
  (let ((inner (fiber/new |:error| (fn [] (emit 1 "err")))))
    (let ((outer (fiber/new |:error| (fn []
                     (fiber/resume inner)
                     (fiber/propagate inner)))))
      (fiber/resume outer)
...
(begin
  (let ((c (fiber/new || (fn [] (emit 1 "deep error")))))
    (let ((b (fiber/new || (fn [] (fiber/resume c)))))
      (let ((a (fiber/new |:error| (fn [] (fiber/resume b)))))
        (fiber/resume a)
...
(begin
  (let ((inner (fiber/new || (fn [] (emit 1 "err")))))
    (let ((outer (fiber/new |:error| (fn []
                     (fiber/resume inner)
                     42))))
      (fiber/resume outer)
...
(begin
  (let ((f (fiber/new |:error| (fn [] (emit 1 "oops") "recovered"))))
    (fiber/resume f)
...
(begin
  (defn helper (n)
    (emit 2 n)
    (helper (+ n 10)))
  (let* ((f (fiber/new |:yield| (fn [] (helper 1)))))
...
(begin
  ...
  (let* ((f (fiber/new |:yield| (fn [] (bouncer 100)))))
...
(let (([ok? _] (protect ((fn ()
  (let ((inner (fiber/new |:error| (fn () (emit 1 "boom")))))
    (fiber/resume inner)
...
(begin
  (var fibers @[])
  (let ([f (fiber/new |:yield| (fn [] (emit 2 :ping) :done))])
    (push fibers f))
...
(begin
  (var store @[])
  (let ([a (fiber/new || (fn [] 1))])
    (push store a))
  (let ([b (fiber/new || (fn [] 2))])
    (push store b))
...
(begin
  (var bucket @[])
  (let ([pid 0]
        [f (fiber/new |:error :yield| (fn [] (emit 2 :hello) :world))])
    (push bucket f)
