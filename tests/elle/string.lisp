...
(assert (= (begin
             ...
             (pop b))
           "i")
        "pop returns last grapheme as string")
(assert (= (begin
             (def @b (thaw "café"))
             (pop b))
           "é")
        "pop returns last multibyte grapheme")
(let [[ok? _] (protect ((fn ()
                          (begin
                            (def @b (thaw ""))
                            (pop b)))))]
...
(assert (= (freeze (begin
                     ...
                     b))
           "hi!")
        "push appends string to @string")
(assert (= (freeze (begin
                     (def @b (thaw "café"))
                     (push b "x")
                     b))
           "caféx")
        "push appends to multibyte @string")
(let [[ok? _] (protect ((fn ()
                          (begin
                            (def @b (thaw "hi"))
                            (push b 33)))))]
...
(assert (string? (begin
                   ...
                   (remove b 1 2)
                   b))
        "remove multiple returns @string")
