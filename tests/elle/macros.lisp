...
(begin
  (assert (= (-> 5 (+ 3)) 8) "thread-first single positive"))

(begin
  (assert (= (-> -10 (+ 5)) -5) "thread-first single negative"))

(begin
  (assert (= (-> 0 (+ 0)) 0) "thread-first single zero"))

# (-> v (+ a) (* b)) is equivalent to (* (+ v a) b)
(begin
  (assert (= (-> 2 (+ 3) (* 4)) 20) "thread-first chain positive"))

(begin
  (assert (= (-> -5 (+ 10) (* 2)) 10) "thread-first chain mixed"))

(begin
  (assert (= (-> 0 (+ 0) (* 5)) 0) "thread-first chain zero"))

# thread-first with three operations
(begin
  (assert (= (-> 1 (+ 2) (* 3) (- 1)) 8) "thread-first three ops positive"))

(begin
  (assert (= (-> -2 (+ 5) (* 2) (- 3)) 3) "thread-first three ops mixed"))

(begin
  (assert (= (-> 0 (+ 1) (* 2) (- 0)) 2) "thread-first three ops zero"))
...
(begin
  (assert (= (->> 3 (- 10)) 7) "thread-last single positive"))

(begin
  (assert (= (->> 5 (- -10)) -15) "thread-last single negative"))

(begin
  (assert (= (->> 0 (- 0)) 0) "thread-last single zero"))

# (->> v (- a) (- b)) is equivalent to (- b (- a v))
(begin
  (assert (= (->> 2 (- 10) (- 5)) -3) "thread-last chain positive"))

(begin
  (assert (= (->> -3 (- 5) (- -2)) -10) "thread-last chain mixed"))

(begin
  (assert (= (->> 0 (- 0) (- 0)) 0) "thread-last chain zero"))
...
(begin
  (assert (= (block 1 2 3) 3) "block returns last positive"))

(begin
  (assert (= (block -5 -10 -3) -3) "block returns last negative"))

(begin
  (assert (= (block 0 0 0) 0) "block returns last zero"))
...
(begin
  (assert (= (block 1 2 3) 3) "block multiple exprs positive"))

(begin
  (assert (= (block -10 -5 -1) -1) "block multiple exprs negative"))

(begin
  (assert (= (block 0 0 0) 0) "block multiple exprs zero"))
...
(begin
  ...
  (assert (= (when-true false 42) nil) "test_macro_with_conditional_body_regression"))
...
(begin
  (assert (= (syntax->datum 42) 42) "test_syntax_to_datum_non_syntax_passthrough"))
...
(assert (not (test-kw? foo)) "syntax-keyword? on symbol — is syntax, not keyword")
