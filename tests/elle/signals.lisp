...
(begin
  (signal :rt_c5a)
  (assert-eq ((squelch (fn () nil) :rt_c5a)) nil
...
(begin
  ...
  (assert-false ok-prim?
    "squelch runtime: non-closure produces type error"))

# squelch runtime: a silent closure passes squelch
(begin
  (assert-eq ((squelch (fn () 99) :yield)) 99
...
(begin
  (signal :audit_sq_c5b)
  (assert-eq ((squelch (fn () 42) :yield)) 42
