...
# tail call. The file makes eleven recursions at about 8 µs a call with the JIT
# off, so this depth keeps it inside the per-file time budget.
#
# The compiled tier's hand-off to the interpreter on a low native stack is not
# pinned here: whether the eager run reaches it depends on when the JIT worker
# finishes. tests/integration/deep_recursion.rs compiles on the VM thread and
# pins it 100,000 deep.
#
...
(defn sum-to [n]
  (if (= n 0)
    0
    (+ n (sum-to (- n 1)))))
(assert (= (sum-to depth) 50005000) "non-tail sum 10,000 deep")
...
(defn build [n]
  (if (= n 0)
    ...
    (pair n (build (- n 1)))))
(let [xs (build depth)]
  (assert (= (length xs) depth) "non-tail list build 10,000 deep")
  ...
  (letrec [ping (fn [m]
           ...
           pong (fn [m]
                  (if (= m 0)
                    ...
                    (+ 1 (ping (- m 1)))))]
    (ping n)))
(assert (= (ping-pong depth) depth) "non-tail mutual recursion 10,000 deep")
...
(defn splice-sum [n]
  (if (= n 0)
    0
    (+ n (splice-sum ;[(- n 1)]))))
(assert (= (splice-sum depth) 50005000) "non-tail spliced recursion 10,000 deep")

## ── A yield at the bottom parks every paused caller ────────────────────
## The resume value comes back up through all 10,000 callers, so the result
...
(let [f (fiber/new (fn [] (dive depth)) |:yield|)]
  (assert (= (fiber/resume f nil) :bottom)
          "a yield 10,000 calls deep reaches the resumer")
