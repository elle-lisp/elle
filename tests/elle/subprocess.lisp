...
(assert (= (ev/spawn (fn [] (subprocess/wait (subprocess/exec "true" [])))) 0) "subprocess/wait: /bin/true exits 0")

# subprocess/wait: exit 1
(assert (= (ev/spawn (fn [] (subprocess/wait (subprocess/exec "false" [])))) 1) "subprocess/wait: /bin/false exits 1")
...
(assert (= (ev/spawn (fn [] (get (subprocess/system "echo" ["hello"]) :exit))) 0) "subprocess/system: echo exits 0")

# subprocess/system: stdout captured
(assert (= (ev/spawn (fn [] (get (subprocess/system "echo" ["hello"]) :stdout))) "hello\n") "subprocess/system: echo stdout")

# subprocess/system: stderr captured and empty
(assert (= (ev/spawn (fn [] (get (subprocess/system "echo" ["hello"]) :stderr))) "") "subprocess/system: echo stderr is empty")
...
(assert (= (ev/spawn (fn []
    (subprocess/wait (subprocess/exec "true" ())))) 0) "subprocess/exec: empty list args work")
...
(let (([ok? err] (protect ((fn [] (ev/spawn (fn [] (subprocess/exec "echo" "not-a-sequence")))))))) (assert (not ok?) "subprocess/exec: string args gives type-error") (assert (= (get err :error) :type-error) "subprocess/exec: string args gives type-error"))

# subprocess/exec rejects non-string element in list with type-error
(let (([ok? err] (protect ((fn [] (ev/spawn (fn [] (subprocess/exec "echo" (cons 42 ()))))))))) (assert (not ok?) "subprocess/exec: non-string element in list gives type-error") (assert (= (get err :error) :type-error) "subprocess/exec: non-string element in list gives type-error"))
...
(assert (= (ev/spawn (fn [] (get (subprocess/system "echo" (list "hi")) :stdout))) "hi\n") "subprocess/system: list args work")

# subprocess/system accepts empty list
(assert (= (ev/spawn (fn [] (get (subprocess/system "true" ()) :exit))) 0) "subprocess/system: empty list args work")

# subprocess/system accepts @array for args
(assert (= (ev/spawn (fn [] (get (subprocess/system "echo" @["bye"]) :stdout))) "bye\n") "subprocess/system: @array args work")
