...
# the next length prefix and produces a parse error.
#
...
# the connection block stays shallow enough to read.
(defn run-race []
    ...
    (while (< f n-fibers)
      (let [@k 0]
        (while (< k n-keys-per-round)
          (redis:set (string key-prefix f ":" k) (string "value-" f "-" k))
          (assign k (+ k 1))))
      (assign f (+ f 1))))
  (println "  seeded " (* n-fibers n-keys-per-round) " keys")

  # Run M fibers, each doing N rounds of a multi-key pipeline.  Each fiber
  # records pass/fail in shared state and we tally at the end.  Any fiber
  # seeing wrong values, RESP-level errors, or a pipeline-length mismatch
  # flips ok to false.
  (def @ok true)
  (def @fail-msgs @[])
  (def @done-count 0)
                              ...
                              (begin
                                ...
                                (if (not (= (length results) n-keys-per-round))
                                  (begin
                                    ...
                                    (push fail-msgs
                                    (string "fiber " fiber-idx " round " r
                                    ": pipeline returned " (length results)
                                    " replies, want " n-keys-per-round)))
                                  (let [@kk 0]
                                    (while (< kk n-keys-per-round)
                                      (let [want (string "value-" fiber-idx "-"
                                        kk)
                                        got (get results kk)]
                                        (when (not (= got want))
                                          (assign ok false)
  ...
  # Wait for all fibers to finish — cooperative, so yield until the counter
  # reaches the target.
  ...
  (if ok
    (println "  " n-fibers " fibers × " n-rounds " rounds × " n-keys-per-round
             " keys: all framing intact")
    (begin
...
(gate! (service-up? "127.0.0.1" 6379) "redis not running on 127.0.0.1:6379"
       (println "  redis: available") (redis:with "127.0.0.1" 6379 run-race)
       (println "") (println "redis-race test passed."))
