...
(assert (= (debug-print "hello") "hello") "debug-print returns the value (string)")
...
(assert (int? (+ (current-thread-id) 1)) "current-thread-id can be used in arithmetic")

# debug-print with list operations
(assert (= (debug-print (list 1 2 3)) (list 1 2 3)) "debug-print works with list operations")
...
(assert (= (begin (debug-print 1) (debug-print 2) (debug-print 3)) 3) "multiple debug-prints return last value")
...
(assert (= (current-thread-id) (current-thread-id)) "multiple calls to current-thread-id return same value")

# debug-print with nested structures
(assert (fn () (debug-print (list (list 1 2) (list 3 4)))) "debug-print with nested lists")
...
(assert (fn () (match 5 (5 "five") (_ nil))) "match syntax is properly parsed")

# match wildcard catches any
(assert (fn () (match 42 (_ "matched"))) "wildcard matches int")
...
(assert (fn () (match 5 (5 true) (5 false) (_ nil))) "first matching clause is used")

# match default wildcard
(assert (fn () (match 99 (1 "one") (2 "two") (_ "other"))) "wildcard matches when no literals match")

# match nil pattern parsing
(assert (fn () (match nil (nil "empty") (_ nil))) "nil pattern parses and works")

# match wildcard pattern
(assert (= (match 42 (_ "any")) "any") "match wildcard with int")
(assert (= (match "hello" (_ "matched")) "matched") "match wildcard with string")

# match nil pattern
(assert (= (match nil (nil "empty") (_ nil)) "empty") "match nil pattern matches nil")
(assert (= (match (list) (nil "empty") (_ "not-nil")) "not-nil") "nil pattern does not match empty list")

# match default case
(assert (= (match 99 (1 "one") (2 "two") (_ "other")) "other") "default pattern catches unmatched values")

# match multiple clauses ordering
(assert (= (match 2 (1 "one") (2 "two") (3 "three") (_ nil)) "two") "match clause ordering: 2 matches second clause")
(assert (= (match 1 (1 "one") (2 "two") (3 "three") (_ nil)) "one") "match clause ordering: 1 matches first clause")

# match with static expressions
(assert (= (match 10 (10 (* 2 3)) (_ nil)) 6) "match evaluates result expression (multiply)")
(assert (= (match 5 (5 (+ 1 1)) (_ nil)) 2) "match evaluates result expression (add)")

# match string literals
(assert (= (match "hello" ("hello" "matched") (_ "no")) "matched") "match string literals")
...
(assert (fn () (import-file "tests/modules/test.lisp")) "import-file with variable definitions")

# import multiple files sequentially
(assert (fn () (import-file "tests/modules/test.lisp")) "first import-file succeeds")
...
(assert (fn () (import-file "./tests/modules/test.lisp")) "import-file with ./ relative path")
(assert (fn () (import-file "tests/modules/test.lisp")) "import-file with relative path")
...
# match array literal
(assert (= (match [1 2 3] ([1 2 3] "exact") (_ "no")) "exact") "match exact array literal")

# match array binding
(assert (= (match [10 20] ([a b] (+ a b)) (_ 0)) 30) "match array with binding")

# match array wrong length
(assert (= (match [1 2] ([a b c] "three") ([a b] "two") (_ nil)) "two") "match array wrong length falls through")

# match array not array
(assert (= (match 42 ([a b] "array") (_ "other")) "other") "match non-array falls through")

# match array empty
(assert (= (match [] ([] "empty") (_ "other")) "empty") "match empty array")

# match array rest
(assert (= (match [1 2 3 4] ([a & rest] (length rest)) (_ 0)) 3) "match array with rest captures remaining")

# match array nested
(assert (= (match [1 [2 3]] ([a [b c]] (+ a (+ b c))) (_ 0)) 6) "match nested arrays")
...
# match guard basic
(assert (= (match 5 (x when (> x 3) "big") (x "small")) "big") "guard passes when condition true")
(assert (= (match 2 (x when (> x 3) "big") (x "small")) "small") "guard falls through when condition false")

# match guard with literal
(assert (= (match 10 (10 when false "nope") (10 "yes") (_ nil)) "yes") "guard with literal falls through on false")
...
# match cons pattern
(assert (= (match (cons 1 2) ((h . t) (+ h t)) (_ 0)) 3) "match cons pattern")

# match cons not pair
(assert (= (match 42 ((h . t) "pair") (_ "nope")) "nope") "match non-pair falls through")
...
# match list rest
(assert (= (match (list 1 2 3) ((a & rest) a) (_ nil)) 1) "match list rest captures first")

# match list exact length
(assert (= (match (list 1 2 3) ((1 2) "two") ((1 2 3) "three") (_ nil)) "three") "match list exact length")
...
(assert (= (match :foo (:foo "matched") (_ "no")) "matched") "match keyword literal matches")
(assert (= (match :bar (:foo "matched") (_ "no")) "no") "match keyword literal doesn't match different keyword")
...
(assert (= (match 42 (x (+ x 1))) 43) "match variable binding")
...
(assert (= (begin (defmacro my-list (& items) `(list ,;items)) (my-list 1 2 3)) (list 1 2 3)) "variadic macro basic")

# variadic macro fixed and rest
(assert (= (begin (defmacro my-add (first & rest) `(+ ,first ,;rest)) (my-add 1 2 3)) 6) "variadic macro with fixed and rest")
...
(assert (= (match (cons 1 (cons 2 3)) ((a b . c) (list a b c)) (_ :no)) (list 1 2 3)) "match improper list pattern")

# match improper list pattern longer
(assert (= (match (list 1 2 3 4 5) ((a b c . d) (list a b c d)) (_ :no)) (list 1 2 3 (list 4 5))) "match improper list pattern longer")

# match improper list pattern exact
(assert (= (match (cons 1 2) ((a . b) (list a b)) (_ :no)) (list 1 2)) "match improper list pattern exact")

# match improper list pattern too short
(assert (= (match (list 1) ((a b . c) :matched) (_ :no)) :no) "match improper list pattern too short")
...
# or pattern basic
(assert (= (match 2 ((or 1 2 3) :small) (_ :big)) :small) "or pattern basic match")

# or pattern no match
(assert (= (match 5 ((or 1 2 3) :small) (_ :big)) :big) "or pattern no match")

# or pattern keywords
(assert (= (match :b ((or :a :b :c) :found) (_ :not)) :found) "or pattern with keywords")

# or pattern with binding
(assert (= (match (cons 1 2) ((or (x . _) (_ . x)) x) (_ 0)) 1) "or pattern with binding")
...
(assert (= (match (cons 2 :x) (((or 1 2) . t) t) (_ :fail)) :x) "or pattern nested in cons")

# or pattern two alternatives
(assert (= (match :y ((or :x :y) :found) (_ :not)) :found) "or pattern two alternatives")

# or pattern with nil
(assert (= (match nil ((or nil 0) :empty) (_ :other)) :empty) "or pattern with nil")

# or pattern in tuple
(assert (= (match [2 :x] ([(or 1 2) y] y) (_ :fail)) :x) "or pattern in tuple")
...
(assert (= (match 10 (x when (> x 5) :big) (x :small)) :big) "guard references pattern var: big")
(assert (= (match 3 (x when (> x 5) :big) (x :small)) :small) "guard references pattern var: small")

# guard fallthrough
(assert (= (match 5 (x when false :never) (x :always)) :always) "guard fallthrough")

# guard with cons
(assert (= (match (cons 1 2) ((h . t) when (> h 0) (+ h t)) (_ 0)) 3) "guard with cons")

# guard with list
(assert (= (match (list 1 2 3) ((a b c) when (> (+ a b c) 5) :big) (_ :small)) :big) "guard with list")

# guard with tuple
(assert (= (match [1 2] ([a b] when (< a b) :ordered) (_ :no)) :ordered) "guard with tuple")

# guard with struct
(assert (= (match {:x 10 :y 20} ({:x x :y y} when (> y x) :valid) (_ :no)) :valid) "guard with struct")

# guard with rest
(assert (= (match (list 1 2 3) ((a & rest) when (> a 0) rest) (_ :fail)) (list 2 3)) "guard with rest")
...
(assert (= (match 2 ((or 1 2 3) when false :never) (_ :fallback)) :fallback) "or pattern guard fallthrough")
...
(assert (= (match 42 (1 :one) (_ :other)) :other) "exhaustive match with wildcard")
...
(assert (= (match 5
              ...
              (5 :unguarded)
              (_ :default)) :unguarded) "decision tree guard fallthrough to next constructor")
...
(each i (list 1 2 3)
  (assign test-result (cons (match i
                           ...
                           (_ :other))
                         test-result)))
(assert (= (reverse test-result) (list :one :two :three)) "decision tree match in loop")
...
(assert (= (match (cons 1 :x) ((1 . t) t) ((2 . t) t) (((or 3 4) . t) t) (_ :fail)) :x) "or pattern decision tree shared")

# or pattern nested decision tree
(assert (= (match [3 :y] ([(or 1 2 3) v] v) (_ :fail)) :y) "or pattern nested decision tree")

# or pattern guard decision tree
(assert (= (match 5 ((or 1 2 3) when true :small) ((or 4 5 6) :medium) (_ :big)) :medium) "or pattern guard decision tree")
