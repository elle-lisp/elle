...
(process:start (fn []
  (let* [[listener lport] (listen-ephemeral)]
    ...
    (let [conn (tcp/connect "127.0.0.1" lport)
      ...
      (assert (= data (bytes 72 73)) "got HI from sub-fiber server")
      (port/close conn)
      (port/close listener)))))
...
(process:start (fn []
  (let* [[listener lport] (listen-ephemeral)]
    ...
    (let [c1 (tcp/connect "127.0.0.1" lport)
          ...
          d2 (port/read c2 1)]
      (assert (or (= d1 (bytes 65)) (= d1 (bytes 66))) "conn1 got data")
      (assert (or (= d2 (bytes 65)) (= d2 (bytes 66))) "conn2 got data")
      (port/close c1)
      (port/close c2)
      (port/close listener)))))
  ...
  (let* [[listener lport] (listen-ephemeral)]
    (ev/spawn (fn []
      (let [conn (tcp/accept listener)]
        (let [greeting (port/read conn 5)]
          (assert (= greeting (bytes 72 69 76 76 79)) "server got HELLO"))
    ...
    (let [conn (tcp/connect "127.0.0.1" lport)]
      ...
      (let [resp (port/read conn 5)]
        (assert (= resp (bytes 87 79 82 76 68)) "client got WORLD"))
...
(process:start (fn []
  (let* [[listener lport] (listen-ephemeral)]
    ...
    (let [conn (tcp/connect "127.0.0.1" lport)
      ...
      (assert (= data (bytes 78 69 83 84)) "got NEST from sub-sub-fiber")
      (port/close conn)
      (port/close listener)))))
...
(process:start (fn []
  (let* [[listener lport] (listen-ephemeral)]
    ...
    (let [conn (tcp/connect "127.0.0.1" lport)]
      (assert (not (nil? conn)) "tcp connect to h2 server succeeded")
      (port/close conn)
      (port/close listener)))))
...
(def grpc ((import "std/grpc") :http2 http2
...
(process:start (fn []
  (let* [[listener lport] (listen-ephemeral)
         url (concat "http://127.0.0.1:" (string lport))]
    (ev/spawn (fn [] (protect (http2:serve listener grpc-handler))))
    (let [session (http2:connect url)]
      (defer (begin (protect (http2:close session))
                    (protect (port/close listener)))
        (let [raw (grpc:call session nil "/test.Svc/Echo" "test.Req" {})]
    ...
    (let [session (http2:connect url)]
      (defer (begin (protect (http2:close session))
        ...
        (let [f1 (ev/spawn (fn []
          ...
          (let [r1 (ev/join f1)
                r2 (ev/join f2)]
            (assert (= r1:status 200) "concurrent req 1: status 200")
            (assert (= r2:status 200) "concurrent req 2: status 200"))))))))
...
(process:start (fn []
  (let* [[listener lport] (listen-ephemeral)
    ...
    (process:spawn (fn []
      (http2:serve listener h2-handler)))
