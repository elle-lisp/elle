                 ...
                 (let* [[listener lport] (listen-ephemeral)
                   ...
                   (let [session (http2:connect url)]
                     (let [resp (http2:send session "GET" "/cross-fiber")]
                       (assert (= resp:status 200) "cross-fiber: got response"))  # close from a sub-fiber
                     (ev/join (ev/spawn (fn [] (http2:close session))))
                     (port/close listener)))))
                   ...
                   (let [session (http2:connect url)]
                     (defer
                       (begin
                         (protect (http2:close session))
                         (protect (port/close listener)))
                       (let [result (ev/timeout 5
                             (fn [] (http2:send session "GET" "/timeout-test")))]
                         (assert (not (nil? result))
                                 "ev/timeout: completed before timeout")
...
(process:start (fn []
                 (let* [[listener lport] (listen-ephemeral)
                        url (concat "http://127.0.0.1:" (string lport))  # 128KB body — exceeds 65535 default window, but within negotiated 1MB
                        body (apply concat
                                    (map (fn [_] (bytes 65 66 67 68 69 70 71 72))
                                    (range 16384)))]
                   (ev/spawn (fn []
...
(process:start (fn []
                 (let* [[listener lport] (listen-ephemeral)
                        me (process:self)]
                   (process:spawn (fn [] (http2:serve listener h2-handler)))  # parent process is the client
