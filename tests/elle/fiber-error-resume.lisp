...
(let [f (fiber/new (fn [] (+ 1 (emit :error {:reason :divide-by-zero})))
  ...
  (assert (= recovered 43) "recovery value flows back into computation"))
(println "  1. ok")
...
(let [inner (fiber/new (fn [] (+ 1 (emit :error {:reason :oops}))) |:yield|)]
  (let [outer (fiber/new (fn [] (fiber/resume inner)) |:error|)]
    (def err-val (fiber/resume outer))
    (assert (= (fiber/status inner) :error) "inner fiber is in :error state")
    (assert (= err-val:reason :oops) "error value is accessible")
...
(let [f (fiber/new (fn []
                     ...
                     (while true
                       ...
                       (let [v (emit :error {:reason :need-input :attempt count})]
                         (assign total (+ total v))
                         (when (>= count 3) (break total)))))
                   |:error|)]
