...
(def fake-tls
  {:connect (fn [host port]
              (push-log :connect host port)
              :fake-conn)
   :read (fn [conn n]
           (push-log :read conn n)
           nil)
   :read-line (fn [conn]
                (push-log :read-line conn)
                nil)
   :write (fn [conn data]
            (push-log :write conn data)
            (length data))
...
(defn canned-read-line [conn]
  ...
  (let [i (get canned-cursor 0)]
    ...
    (when (< i (length canned-response)) (get canned-response i))))
...
(def fake-compress
  {:gzip (fn [data & opts]
           (push compress-log [:gzip data opts])
           (bytes (string "GZ:" data)))
   :gunzip (fn [data]
             (push compress-log [:gunzip data])
             (bytes "ungz"))
   :zlib (fn [data & opts]
           (push compress-log [:zlib data opts])
           (bytes (string "ZL:" data)))
   :unzlib (fn [data]
             (push compress-log [:unzlib data])
             (bytes "unzl"))
   :deflate (fn [data & opts]
              (push compress-log [:deflate data opts])
              (bytes (string "DF:" data)))
   :inflate (fn [data]
              (push compress-log [:inflate data])
              (bytes "infl"))
   :zstd (fn [data & opts]
           (push compress-log [:zstd data opts])
           (bytes (string "ZD:" data)))
   :unzstd (fn [data]
             (push compress-log [:unzstd data])
             (bytes "unzd"))})
...
# Spawn server
(def server-fiber (ev/spawn (fn [] (http:serve listener test-handler))))
...
(def chunk-fiber (ev/spawn (fn [] (http:serve chunk-listener chunked-handler))))
...
(defn redir-handler [req]
  ...
  (case
    req:path  # /hop1 → /hop2 (302, rewrites to GET)
    "/hop1" {:status 302
    ...
    # /end: terminal
    "/end" (http:respond 200 (string req:method " /end"))
...
(def redir-fiber (ev/spawn (fn [] (http:serve redir-listener redir-handler))))
