...
(redis:with "127.0.0.1" 6379
  (fn []
    ...
    (redis:set "test:p1" "x")
    (redis:set "test:p2" "y")
    (let [results (redis:pipeline (list "GET" "test:p1") (list "GET" "test:p2")
            (list "PING"))]
    ...
    (def @i 0)
    (while (< i 100)
      (assert (= (redis:ping) "PONG") (concat "ping failed at " (string i)))
      (assign i (+ i 1)))
    ...
    (while (< i 50)
      (let [key (concat "test:sg:" (string i))
        ...
        (assert (= (redis:get key) val) (concat "get failed at " (string i))))
      (assign i (+ i 1)))
