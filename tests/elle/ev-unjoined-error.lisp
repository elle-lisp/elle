...
(let [[[ok? val] (protect
                   (let [[f (ev/spawn (fn [] (error {:error :joined :message "propagated"})))]]
                     (ev/join f)))]]
...
(let [[[ok? val] (protect
                   (ev/scope (fn [spawn]
                     (spawn (fn [] (error {:error :scoped :message "in scope"})))
                     (ev/sleep 0.01))))]]
...
(let* [[elle  (or (get (sys/env) "ELLE") "./target/debug/elle")]
  ...
  (let [[result (subprocess/system elle [tmp])]]
    (assert (not (= (get result :exit) 0)) "unjoined error must crash (non-zero exit)")
...
(let* [[elle  (or (get (sys/env) "ELLE") "./target/debug/elle")]
  ...
  (port/close p)
  (let [[result (subprocess/system elle [tmp])]]
    (assert (= (get result :exit) 0) "successful unjoined fiber should not crash")))
