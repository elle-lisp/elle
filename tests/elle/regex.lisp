...
(let [[ok? _] (protect ((fn () (compile-fn "[invalid"))))]
  (assert (not ok?) "regex/compile invalid pattern"))

(let [[ok? _] (protect ((fn () (compile-fn 42))))]
  (assert (not ok?) "regex/compile wrong type"))

(let [[ok? _] (protect ((fn () (compile-fn))))]
  (assert (not ok?) "regex/compile wrong arity: no args"))

(let [[ok? _] (protect ((fn () (compile-fn "a" "b"))))]
  (assert (not ok?) "regex/compile wrong arity: two args"))
...
(let [[ok? _] (protect ((fn () (match-fn "not-a-regex" "abc"))))]
  (assert (not ok?) "regex/match? wrong type"))
