...
(while (%lt i 2000)
  (let* [r2 (map (fn [u] u)
    ...
    (assert (= (length r2) 4)
            "the map-built list survives a tail-recursive match walk")
    (assert (= (length (w_rest r2)) 3)
            "cons Rest alias survives the owned-param self-call")
    (assert (= (w_first r2) "a")
            "cons First (head) alias survives the owned-param self-call")
    (assert (= (w_index arr2) 1)
            "array Index element alias survives the owned-param self-call")
    (assert (= (w_key {:k 99 :j 88}) 99)
            "struct Key value alias survives the owned-param self-call")
    (assert (= (w_order_driver r2) 3)
            "ordering: a match-compound tail arg is classified after its aliases are registered")
    (assert (= (length (w_destructure_rest r2)) 3)
            "destructure rest alias survives the owned-param self-call")
    (assert (= (w_destructure_head r2) {:name "a"})
            "destructure head alias survives the owned-param self-call"))
  (assign i (%add i 1)))
