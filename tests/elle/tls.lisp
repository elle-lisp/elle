...
(let [cert-path "/tmp/elle-tls-test.cert.pem"
  ...
  (let [gen-result (subprocess/system "openssl"
          ["req" "-x509" "-newkey" "rsa:2048" "-keyout" key-path "-out"
           cert-path "-days" "1" "-nodes" "-subj" "/CN=localhost"])]
    (if (not (= gen-result:exit 0))
      ...
      (begin  ## Shared mutable cell: the client fiber writes true here when done.
        ...
        (def client-fiber
          (ev/spawn (fn []
                      (let [conn (tls:connect "127.0.0.1" server-port
                              {:no-verify true})]
                        (defer (tls:close conn) (tls:write conn "hello\n")
        ...
        (assert (get loopback-ok 0)
          "tls loopback: client fiber must complete and verify response")
        (port/close listener)
...
(let [[ok? err] (protect ((fn [] (tls:connect "127.0.0.1" 19999))))]
  (assert (not ok?) "connect to closed port: must signal an error")
  (assert (or (= (get err :error) :io-error) (= (get err :error) :connect-error)
...
(if has-network
  (begin
    (let [[ok? err] (protect ((fn [] (tls:connect "example.com" 80))))]
      (assert (not ok?) "connect to plain HTTP port: must signal an error")
      (assert (or (= (get err :error) :tls-error) (= (get err :error) :io-error)
