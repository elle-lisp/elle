...
#
# The cases vary what the teardown has to interrupt — payload size, the
# server's own latency, how tight the recycle is, whether the streams run
# concurrently, and whether the server streams its response back rather
# than buffering it. The last two use no h2 at all: they put the same
# `ev/timeout` churn on a bare queue, so a failure there separates the
