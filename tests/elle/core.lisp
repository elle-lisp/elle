...
# test_append
(assert (= (append (append (list 1 2) (list 3 4)) (list 5)) (list 1 2 3 4 5)) "append lists")
...
(assert (= ((fn [x] (if (> x 50) (* x 2) (+ x 100))) 25) 125) "closure param in nested expr")
...
(assert (= ((fn [n] (if (nil? n) "empty" "nonempty")) (list 1)) "nonempty") "closure param in conditional")
...
(assert (= (-> 5 (+ 10) (* 2)) 30) "thread-first simple")

# test_thread_first_with_multiple_args
(assert (= (-> 5 (+ 10 2) (* 3)) 51) "thread-first multiple args")

# test_thread_last_simple
(assert (= (->> 5 (+ 10) (* 2)) 30) "thread-last simple")

# test_thread_last_with_multiple_args
(assert (= (->> 2 (+ 10) (* 3)) 36) "thread-last multiple args")

# test_thread_first_chain
(assert (= (-> 1 (+ 1) (+ 1) (+ 1)) 4) "thread-first chain")

# test_thread_last_chain
(assert (= (->> 1 (+ 1) (+ 1) (+ 1)) 4) "thread-last chain")

# test_thread_first_with_list_ops
(assert (= (-> (list 1 2 3) (length)) 3) "thread-first list ops")

# test_thread_last_with_list_ops
(assert (= (->> (list 1 2 3) (length)) 3) "thread-last list ops")

# test_thread_first_nested
(assert (= (-> 10 (- 3) (+ 5)) 12) "thread-first nested")

# test_thread_last_nested
(assert (= (->> 10 (- 3) (+ 5)) -2) "thread-last nested")
...
(assert (= (some-> 5 (+ 1) (* 2)) 12) "some-> simple chain")

# test_some_thread_first_nil_input
(assert (= (some-> nil (+ 1)) nil) "some-> short-circuits on nil input")

# test_some_thread_first_nil_midchain
(def some-test-fn (fn (x) nil))
(assert (= (some-> 5 some-test-fn (+ 1)) nil) "some-> short-circuits mid-chain")

# test_some_thread_first_false_passes_through
# false is falsy but not nil; some-> must not short-circuit on false
(assert (= (some-> false not) true) "some-> false is not nil, passes through")
...
(assert (= (some->> 2 (- 10) (* 3)) 24) "some->> inserts value as last argument")
...
# test_min_max_float
(assert (< (abs (- (min 1.5 2 0.5) 0.5)) 0.0000000001) "min float: min(1.5,2,0.5)=0.5")

# test_abs_float
(assert (< (abs (- (abs -3.5) 3.5)) 0.0000000001) "abs float: abs(-3.5)=3.5")

# test_type_conversions_float
(assert (< (abs (- (float 5) 5.0)) 0.0000000001) "float conversion: (float 5)=5.0")
