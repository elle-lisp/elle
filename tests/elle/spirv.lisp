...
(let* [bytecode (spv:compute 256 2
         (fn [s]
           (let* [id (s:global-id)
                  a (s:load 0 id)
                  b (s:load 1 id)]
             (s:store 1 id (s:fadd a b)))) f32-bits)
       len (length bytecode)]
  (assert (> len 20) "spirv: bytecode has content")  ## Check magic number (first 4 bytes, little-endian)
  (let [magic (bit/or (bytecode 0) (bit/shift-left (bytecode 1) 8)
          (bit/shift-left (bytecode 2) 16) (bit/shift-left (bytecode 3) 24))]
...
(let [bytecode (spv:compute 256 2
        (fn [s]
          (let* [id (s:global-id)
                 ...
                 val (s:load 0 id)]
            (s:store-var v val)
            (s:store 1 id (s:load-var v)))) f32-bits)]
...
(let [bytecode (spv:compute 256 2
        (fn [s]
          (let* [id (s:global-id)
                 ...
                 c (s:const-u 7)]
            (s:store-var v c)
            (s:store 1 id (s:u2f (s:load-var v))))) f32-bits)]
...
(let [bytecode (spv:compute 256 2
        (fn [s]
          (let* [id (s:global-id)
            ...
            (s:branch done)
            (s:begin-block done)
            (s:store 1 id (s:const-f 0.0)))) f32-bits)]
  (assert (> (length bytecode) 20) "spirv: selection merge compiles"))

## ── 7. Control flow: loop ─────────────────────────────────
(let [bytecode (spv:compute 256 1
        (fn [s]
          (let* [id (s:global-id)
                 counter (s:var-u)
                 zero (s:const-u 0)
            ...
            (let* [n (s:load-var counter)
              ...
              (s:branch-cond cmp body done))  ## body
            (s:begin-block body)
            (s:store-var counter (s:iadd (s:load-var counter) one))
            (s:branch cont)  ## continue
            (s:begin-block cont)
            (s:branch hdr)  ## exit
            (s:begin-block done)
            (s:store 0 id (s:u2f (s:load-var counter))))) f32-bits)]
...
(let [bytecode (spv:compute 256 1
        (fn [s]
          (let* [id (s:global-id)
                 ...
                 b (s:const-u 3)
                 c (s:sgt a b)]
            (s:store 0 id (s:select-u c (s:const-u 1) (s:const-u 0))))) f32-bits)]
  (assert (> (length bytecode) 20) "spirv: sgt compiles"))

## ── 10. Mandelbrot-style kernel ───────────────────────────
(let [bytecode (spv:compute 256 3
        (fn [s]
          (let* [id (s:global-id)
                 cx (s:load 0 id)
                 cy (s:load 1 id)
            ...
            (let* [r (s:load-var zr)
              ...
              (s:loop-merge done cont)
              (s:branch-cond go body done))  ## body
            (s:begin-block body)
            (let* [r (s:load-var zr)
                   i (s:load-var zi)
                   ri (s:fmul r i)
            ...
            (s:branch hdr)  ## exit
            (s:begin-block done)
            (s:store 2 id (s:u2f (s:load-var iter))))) f32-bits)]
...
(let [bytecode (spv:compute 256 1
        (fn [s]
          (let* [id (s:global-id)
            ...
            (s:store 0 id (s:u2f or-result)))) f32-bits)]
