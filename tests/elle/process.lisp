  ...
  (let ([me (process:self)])
    (let ([worker-a (process:spawn-link (fn ()
            (let ([b (process:spawn-link (fn ()
                        (error {:error :boom :message "worker-b crashed"})))])
              (process:recv))))])
...
(process:start (fn ()
  (let ([me (process:self)])
    # Spawn a busy-looper that will be preempted by fuel
    (let ([busy (process:spawn (fn ()
            (letrec ([loop (fn [n] (loop (+ n 1)))])
              (loop 0))))])

      # Spawn a worker that sends us a result
      (process:spawn (fn ()
        (process:send me :worker-done)))

      (let ([msg (process:recv)])
        (assert (= msg :worker-done) "preemption: worker completes despite busy-looper")
        # Kill the busy-looper so the scheduler can terminate
...
(process:start (fn ()
  (let* ([me (process:self)]
    ...
    (process:send me :still-alive)
    (let ([msg (process:recv)])
      (assert (= msg :still-alive) "demonitor: no DOWN after demonitor")))))
...
(process:start (fn ()
  (let ([me (process:self)])
    ...
    (let ([pid (process:whereis :echo-server)])
      (assert (not (nil? pid)) "named: whereis finds registered process"))
...
(process:start (fn ()
  ...
  (assert (= (process:get-dict :counter) 42) "dict: get-dict returns current value")
  (assert (nil? (process:get-dict :missing)) "dict: get-dict returns nil for missing key")

  (let ([old (process:erase-dict :counter)])
    (assert (= old 42) "dict: erase-dict returns old value"))

  (assert (nil? (process:get-dict :counter)) "dict: erased key returns nil")))
...
(process:start (fn ()
  (let ([me (process:self)])
    ...
    (let ([msg (process:recv)])
      (assert (= msg :timer-fired) "timer: delayed message arrived")))))
...
(process:start (fn ()
  (let* ([me (process:self)]
    ...
    (let ([msg (process:recv)])
      (assert (= msg :after-cancel) "cancel-timer: cancelled message did not arrive")))))
...
(process:start (fn ()
  (let ([result (process:recv-timeout 1)])
    (assert (= result :timeout) "recv-timeout: times out when no messages"))))
...
(process:start (fn ()
  (let ([me (process:self)])
    ...
    (let ([result (process:recv-timeout 100)])
      (assert (= result :fast) "recv-timeout: returns message when available")))))
...
(process:start (fn ()
  ...
  (let ([child (process:spawn-link (fn ()
                 (process:exit (process:self) [:normal :voluntary])
...
(let ([sched (process:make-scheduler :backend backend)])
  ...
  (process:run sched (fn ()
    (let ([me (process:self)])
      # The scheduler hasn't exposed inject mid-run, but we can test process-info
  ...
  (let ([info (process:process-info sched 0)])
    (assert (= (get info :status) :dead) "external: process-info shows dead after completion")))
...
(process:start (fn ()
  (let* ([f (ev/spawn (fn () (error {:error :boom :message "test"})))]
         [[ok? val] (ev/join-protected f)])
    (assert (not ok?) "ev/join-protected: not ok")
    (assert (= (get val :error) :boom) "ev/join-protected: error is :boom"))))
