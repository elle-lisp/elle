...
(assert-true  (syntax-keyword? syn-kw)   "syntax-keyword? true on syntax keyword")
(assert-false (syntax-keyword? syn-int)  "syntax-keyword? false on syntax int")
(assert-false (syntax-keyword? :bar)     "syntax-keyword? false on plain keyword")
(assert-false (syntax-keyword? 42)       "syntax-keyword? false on plain int")

# ============================================================================
# syntax-nil? — needs a syntax-wrapped nil (not reachable via macro arg)
# ============================================================================

(assert-true  (syntax-nil? syn-nil)  "syntax-nil? true on syntax nil")
(assert-false (syntax-nil? syn-int)  "syntax-nil? false on syntax int")
(assert-false (syntax-nil? nil)      "syntax-nil? false on plain nil")
(assert-false (syntax-nil? 0)        "syntax-nil? false on plain int 0")
...
(let ((result (syntax->list syn-list1)))
  (assert-eq (length result) 1 "syntax->list: length 1 list")
...
(let ((result (syntax->list syn-empty)))
  (assert-eq (length result) 0 "syntax->list: empty list → empty array"))

# Error: non-syntax argument
(assert-err (fn () (syntax->list 42))      "syntax->list: non-syntax errors")

# Error: syntax wrapping a non-list (e.g. an int)
(assert-err (fn () (syntax->list syn-int)) "syntax->list: syntax non-list errors")
...
(let ((elem (syntax-first syn-list2)))
  (assert-eq (syntax-e elem) 1 "syntax-first: returns first element"))

# Error: empty syntax list
(assert-err (fn () (syntax-first syn-empty))  "syntax-first: empty list errors")

# Error: syntax wrapping a non-list
(assert-err (fn () (syntax-first syn-int))    "syntax-first: non-list errors")

# Error: plain non-syntax value
(assert-err (fn () (syntax-first 42))         "syntax-first: non-syntax errors")
...
(let ((tail (syntax-rest syn-list2)))
  (let ((items (syntax->list tail)))
    (assert-eq (length items) 1 "syntax-rest: rest has 1 element")
    (assert-eq (syntax-e (first items)) 2 "syntax-rest: rest element is 2")))

# Error: empty syntax list
(assert-err (fn () (syntax-rest syn-empty))   "syntax-rest: empty list errors")

# Error: syntax wrapping a non-list
(assert-err (fn () (syntax-rest syn-int))     "syntax-rest: non-list errors")

# Error: plain non-syntax value
(assert-err (fn () (syntax-rest 42))          "syntax-rest: non-syntax errors")
...
(assert-eq (syntax-e syn-int)  42      "syntax-e: int unwraps")
(assert-eq (syntax-e syn-bool) true    "syntax-e: bool unwraps")
(assert-eq (syntax-e syn-nil)  nil     "syntax-e: nil unwraps")
(assert-true (= (syntax-e syn-str) "hello") "syntax-e: string unwraps")
...
# Error: non-syntax argument
(assert-err (fn () (syntax-e 42))   "syntax-e: non-syntax errors")
(assert-err (fn () (syntax-e :foo)) "syntax-e: plain keyword errors")
