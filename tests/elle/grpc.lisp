...
(defn test-exports []
  (each key in [:connect :call :call-decode :call-stream :close :encode :decode]
    (assert (not (nil? (get grpc key))) (string "export key exists: " key))))
...
(defn grpc-handler [req]
  ...
  (let [path req:path
    ...
    (cond  ## Echo the body back as gRPC response
      (= path "/test.Svc/Echo")
      ...
      (= path "/test.Svc/Error")
        {:status 200
         :headers {:content-type "application/grpc"}
         :body (bytes)
         :trailers [["grpc-status" "13"] ["grpc-message" "internal error"]]}
      ...
      (= path "/test.Svc/Stream")
        {:status 200
         :headers {:content-type "application/grpc"}
         :body (concat (grpc:encode (bytes 10 11)) (grpc:encode (bytes 20 21))
      ...
      true
        {:status 200
         :headers {:content-type "application/grpc"}
         :trailers [["grpc-status" "12"] ["grpc-message" "unimplemented"]]})))
...
(defn test-grpc-error-in-trailers []
  ...
  (with-server grpc-handler
               (fn [sess]
                 (let [[ok? err] (protect (grpc:call sess nil "/test.Svc/Error"
                       "test.Req" {}))]
...
(defn test-trailers-only-error []
  ...
  (with-server grpc-handler
               (fn [sess]
                 (let [[ok? err] (protect (grpc:call sess nil
                       "/test.Svc/NotFound" "test.Req" {}))]
