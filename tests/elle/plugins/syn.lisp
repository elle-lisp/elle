...
(let [[ok? err] (protect ((fn () (items-fn "not-a-file"))))]
  (assert (not ok?) "items: non-file returns type-error")
  (assert (= (get err :error) :type-error) "items: non-file returns type-error"))
