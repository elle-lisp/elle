...
(let [[ok? err] (protect ((fn [] (file/stat 42))))]
  (assert (not ok?) "file/stat non-string gives type-error")
...
(let [[ok? err] (protect ((fn [] (apply file/stat ["a" "b"]))))]
  (assert (not ok?) "file/stat wrong arity gives arity-error")
...
# non-string argument → type-error
(let [[ok? err] (protect ((fn [] (file/lstat 42))))]
  (assert (not ok?) "file/lstat non-string gives type-error")
  (assert (= (get err :error) :type-error)
          "file/lstat non-string gives type-error"))

# wrong arity → arity-error (use apply to bypass compile-time arity check)
(let [[ok? err] (protect ((fn [] (apply file/lstat ["a" "b"]))))]
  (assert (not ok?) "file/lstat wrong arity gives arity-error")
  (assert (= (get err :error) :arity-error)
...
(assert (not (get stat-info :is-symlink))
        "file/stat through symlink: is-symlink false")
(assert (= (get stat-info :file-type) "file")
        "file/stat through symlink: file-type is file")
...
(assert (= (get lstat-info :file-type) "symlink")
        "file/lstat on symlink: file-type is symlink")
