...
(assert (= (stream/collect (stream/concat (make-from-list (list 1))
                             (make-range 0) (make-from-list (list 2))))
    (list 1 2)) "stream/concat: empty source in middle is skipped")
...
(assert (= (stream/collect (stream/take 2
    ...
    (list 4 6)) "composition: map*2 then filter >3 then take 2 from range 0..9")
