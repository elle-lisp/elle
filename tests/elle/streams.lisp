...
(assert (= (stream/collect (make-from-list (list 1 2 3))) (list 1 2 3)) "stream/collect: three values in order")

# stream/collect: empty coroutine (immediately done)
(assert (= (stream/collect (make-range 0)) ()) "stream/collect: empty source yields empty list")
...
(assert (= (length (stream/into-array (make-range 0))) 0) "stream/into-array: empty source yields empty array")

# === Transform combinators ===

# stream/map: identity transform
(assert (= (stream/collect (stream/map identity (make-range 3))) (list 0 1 2)) "stream/map: identity preserves values")

# stream/map: squaring transform
(assert (= (stream/collect (stream/map (fn [x] (* x x)) (make-range 4))) (list 0 1 4 9)) "stream/map: squares 0..3")

# stream/map: empty source
(assert (= (stream/collect (stream/map (fn [x] (* x 10)) (make-range 0))) ()) "stream/map: empty source yields empty")

# stream/filter: keeps matching values
(assert (= (stream/collect (stream/filter (fn [x] (= (% x 2) 0)) (make-range 6))) (list 0 2 4)) "stream/filter: keeps even values")

# stream/filter: rejects all
(assert (= (stream/collect (stream/filter (fn [x] false) (make-range 3))) ()) "stream/filter: all rejected yields empty")

# stream/filter: keeps all
(assert (= (stream/collect (stream/filter (fn [x] true) (make-range 3))) (list 0 1 2)) "stream/filter: all kept")

# stream/take: fewer than available
(assert (= (stream/collect (stream/take 3 (make-range 10))) (list 0 1 2)) "stream/take: take 3 from 10")

# stream/take: more than available
(assert (= (stream/collect (stream/take 10 (make-range 3))) (list 0 1 2)) "stream/take: take 10 from 3 yields all 3")

# stream/take: zero
(assert (= (stream/collect (stream/take 0 (make-range 5))) ()) "stream/take: take 0 yields empty")

# stream/drop: fewer than available
(assert (= (stream/collect (stream/drop 2 (make-range 5))) (list 2 3 4)) "stream/drop: drop 2 from 5")

# stream/drop: more than available
(assert (= (stream/collect (stream/drop 10 (make-range 3))) ()) "stream/drop: drop more than available yields empty")

# stream/drop: zero
(assert (= (stream/collect (stream/drop 0 (make-range 3))) (list 0 1 2)) "stream/drop: drop 0 yields all")

# stream/concat: two non-empty sources
(assert (= (stream/collect (stream/concat (make-from-list (list 1 2)) (make-from-list (list 3 4)))) (list 1 2 3 4)) "stream/concat: two sources concatenated")

# stream/concat: empty source in the middle
(assert (= (stream/collect (stream/concat
    (make-from-list (list 1))
    (make-range 0)
    (make-from-list (list 2)))) (list 1 2)) "stream/concat: empty source in middle is skipped")
...
(assert (= (stream/collect (stream/zip (make-from-list (list 1 2 3)) (make-from-list (list 4 5 6)))) (list [1 4] [2 5] [3 6])) "stream/zip: same-length sources")

# stream/zip: stops at shortest (first exhausted)
(assert (= (stream/collect (stream/zip (make-range 2) (make-range 5))) (list [0 0] [1 1])) "stream/zip: stops at shortest source")

# stream/zip: one empty source yields empty immediately
(assert (= (stream/collect (stream/zip (make-range 0) (make-range 3))) ()) "stream/zip: empty source causes immediate stop")

# stream/pipe: single transform
(assert (= (stream/collect (stream/pipe (make-range 3) (partial stream/map (fn [x] (* x 2))))) (list 0 2 4)) "stream/pipe: single transform")

# stream/pipe: chained transforms
(assert (= (stream/collect
    (stream/pipe (make-range 10)
      (partial stream/filter (fn [x] (= (% x 2) 0)))
      (partial stream/take 3))) (list 0 2 4)) "stream/pipe: filter then take")

# Composition: map then filter then take
(assert (= (stream/collect
    (stream/take 2
      (stream/filter (fn [x] (> x 3))
        (stream/map (fn [x] (* x 2)) (make-range 10))))) (list 4 6)) "composition: map*2 then filter >3 then take 2 from range 0..9")
...
(assert (= (ev/spawn (fn []
    (stream/collect (port/lines (port/open "/tmp/elle-test-streams-lines-478" :read))))) (list "alpha" "beta" "gamma")) "port/lines: yields all lines from multi-line file")

# port/lines: empty file yields empty list
(spit "/tmp/elle-test-streams-empty-478" "")
(assert (= (ev/spawn (fn []
    (stream/collect (port/lines (port/open "/tmp/elle-test-streams-empty-478" :read))))) ()) "port/lines: empty file yields empty list")

# port/lines: file without trailing newline
(spit "/tmp/elle-test-streams-nonl-478" "no-newline")
(assert (= (ev/spawn (fn []
    (stream/collect (port/lines (port/open "/tmp/elle-test-streams-nonl-478" :read))))) (list "no-newline")) "port/lines: file without trailing newline yields last line")

# port/lines: port is closed after collect exhausts the stream
(spit "/tmp/elle-test-streams-close-478" "one\ntwo")
(assert (not (ev/spawn (fn []
    (let [[p (port/open "/tmp/elle-test-streams-close-478" :read)]]
      (stream/collect (port/lines p))
      (port/open? p))))) "port/lines: port is closed after stream is exhausted")

# port/chunks: basic chunking, 4-byte chunks of 12-byte file
(spit "/tmp/elle-test-streams-chunks-478" "abcdefghijkl")
(assert (= (ev/spawn (fn []
    (stream/collect (port/chunks (port/open "/tmp/elle-test-streams-chunks-478" :read) 4)))) (list "abcd" "efgh" "ijkl")) "port/chunks: 12-byte file in 4-byte chunks")
...
(assert (not (ev/spawn (fn []
    (let [[p (port/open "/tmp/elle-test-streams-chunkclose-478" :read)]]
      ...
      (port/open? p))))) "port/chunks: port is closed after stream is exhausted")
    ...
    (stream/collect
      (stream/take 3
        (stream/map (fn [x] (+ (integer x) 10))
          (port/lines (port/open "/tmp/elle-test-streams-compose-478" :read))))))) (list 11 12 13)) "composition: port/lines -> map -> take -> collect")
      ...
      (stream/take 3
        (stream/filter (fn [row] (> (get row :value) 150))
          (stream/map
            ...
            (stream/drop 1
              (port/lines (port/open "/tmp/elle-test-streams-pipeline-478" :read))))))))) (list {:id 2 :value 200} {:id 3 :value 300} {:id 4 :value 400})) "integration: CSV pipeline drop-header map-parse filter take collect")

# Nested streams: stream/concat of stream/map-ped sources
(assert (= (stream/collect
    (stream/concat
      (stream/map (fn [x] (* x 10)) (make-from-list (list 1 2)))
      (stream/map (fn [x] (* x 100)) (make-from-list (list 3 4))))) (list 10 20 300 400)) "nested streams: concat of mapped sources")
