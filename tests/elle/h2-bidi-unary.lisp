(elle/epoch 12)
# Bidi streams, then ordinary requests on the same session.
#
...
#
# This case spends both and then asks the session to keep working:
# 252 bidi streams, followed by 50 plain requests with bodies on the
# same connection. The requests are the check that the streams left the
# session usable rather than out of window credit or holding ids it
# cannot retire.
#
...
# ── Streams, then ordinary requests on the same session ──────────────
...
(println "bidi streams, then ordinary requests on the same session...")

(timed "252 bidi streams then 50 requests" bidi-then-unary)

(println "h2 bidi unary: the session served requests after its streams")
