...
(assert (= (validate (compile-validator integer?) 42) nil) "predicate: integer? passes for 42")
...
(assert (= (validate (compile-validator (check integer? odd?)) 3) nil) "check: both predicates pass")

# Test 6: check fails on first predicate
(assert (not (nil? (validate (compile-validator (check integer? odd?)) "x"))) "check: fails when first predicate fails")

# Test 7: check fails on second predicate
(assert (not (nil? (validate (compile-validator (check integer? odd?)) 4))) "check: fails when second predicate fails")
...
(let [[f (validate (v/or integer? keyword?) "x")]]
  ...
  (assert (= (length (get f :any)) 2) "v/or: :any has one entry per failing validator"))
...
(assert (= (validate (v/optional integer?) 42) nil) "v/optional: valid non-nil passes")

# Test 18: v/optional fails invalid non-nil value
(assert (not (nil? (validate (v/optional integer?) "x"))) "v/optional: invalid non-nil fails")
...
(assert (= (validate (v/arrayof integer?) []) nil) "v/arrayof: empty array passes")
...
(assert (= (validate (v/mapof keyword? integer?) {}) nil) "v/mapof: empty struct passes")
...
(let [[[ok? err] (protect (safe-add 1 "two"))]]
  ...
  (assert (= (get err :arg) 1) "contract: arg error :arg is 1 (0-indexed second arg)")
