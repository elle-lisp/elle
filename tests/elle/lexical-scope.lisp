...
(assert (= (let [f ((fn (x)
             ...
             (f))
           15)
        "test_let_inside_lambda_capture")
...
(assert (= (let [a 1]
             (let [f ((fn (b)
               ...
               (f)))
           6)
        "test_nested_let_lambda_let")

(assert (= (let [x 5]
             (let [f (fn () x)]
               (f)))
           5)
        "test_lambda_captures_let_binding")
...
(assert (= (let [x 10
             ...
             (let [f1 (fn () x)
               ...
               (+ (f1) (f2))))
           30)
        "test_multiple_lambdas_same_let_scope")

(assert (= (let [outer 100]
             (let [inner 50]
               (let [f (fn () (+ outer inner))]
                 (f))))
           150)
        "test_lambda_in_let_captures_outer_let")
...
(assert (= (let* [x 1
             ...
             (f))
           3)
        "test_let_star_with_lambda_capture")
...
(assert (= ((fn ()
              (begin
                ...
                (begin
                  ...
                  (inc)))))
           3)
        "test_set_on_locally_defined_capture")
...
(assert (= (let [@x 0]
             (let [inc (fn () (assign x (+ x 1)))
               ...
               (begin
                 ...
                 (get))))
           2)
        "test_multiple_closures_share_mutable_capture")
             ...
             (let [f (fn ()
               ...
               (let [g (f)]
                 (begin
                   ...
                   (g)))))
           3)
        "test_mutable_capture_across_lambda_levels")
...
(assert (= (let [@x 0
             ...
             (let [inc-x (fn () (assign x (+ x 1)))
               ...
               (begin
                 ...
                 (sum))))
           3)
        "test_multiple_mutable_captures")
...
(assert (= (let [@counter 0]
             (let [gen (fn ()
               ...
               (let [co (make-coroutine gen)]
                 (coro/resume co))))
           1)
        "test_coroutine_with_mutable_capture")
...
(assert (= (let* [x 5
             ...
             (coro/resume co))
           20)
        "test_coroutine_captures_let_star_binding")
...
(assert (= (let [x 5]
             (let [f (fn () (fn () x))]
               (let [g (f)]
                 (g))))
           5)
        "test_closure_returning_closure_with_captures")
...
(assert (= (let [x 10]
             (let [f (fn (x) (fn () x))]
               (let [g (f 20)]
                 (g))))
           20)
        "test_shadowing_in_nested_scopes")
...
(assert (= (let [x 10]
             (let [f (fn ()
               ...
               (let [g (f)]
                 (g))))
           20)
        "test_capture_with_shadowing_outer")
...
(assert (= (let [x 5]
             (let [f (fn () x)
               ...
               (+ (f) (g))))
           15)
        "test_multiple_captures_same_variable")
...
(assert (= (let [@x 0]
             (let [f (fn ()
               ...
               (begin
                 ...
                 x)))
           3)
        "test_capture_in_loop_body")

# ============================================================================
# SECTION 6: Edge Cases and Stress Tests
# ============================================================================

(assert (= ((fn () 42)) 42) "test_empty_lambda_capture")

(assert (= ((fn (x) 42) 10) 42) "test_lambda_unused_parameter")
...
(assert (= (let [x 10
             ...
             (let [f (fn () x)]
               (f)))
           10)
        "test_capture_unused_let_binding")
...
(assert (= (let [a 1
             ...
             (let [f (fn () (+ a b c d e))]
               (f)))
           15)
        "test_many_captures_same_closure")
...
(assert (= (let* [a 1
             ...
             (f))
           6)
        "test_capture_in_nested_let_star")

(assert (= (let [x 10]
             (let [f (fn (x) (+ x 5))]
               (f 20)))
           25)
        "test_lambda_param_shadows_let_binding")
...
(assert (= (let [x 10]
             (let [f (fn (x) (fn (x) x))]
               (let [g (f 20)]
                 (g 30))))
           30)
        "test_nested_lambda_param_shadowing")
...
(assert (= (let [x 10]
             (let [f (fn ()
               ...
               (f)))
           15)
        "test_capture_with_define_in_lambda")
...
(assert (= (let [limit 4]
             (letrec [is-even (fn (n) (if (= n 0) true (is-odd (- n 1))))
               ...
               (is-even limit)))
           true)
        "test_mutual_recursion_with_captures")
...
(assert (= (let [x 10]
             (begin
               ...
               (f)))
           10)
        "test_capture_across_define_boundary")
...
(assert (= (begin
             ...
             (let [x 100]
               ...
               x))
           100)
        "test_let_inside_closure_does_not_corrupt_caller_stack")
...
(assert (= (begin
             ...
             (f 10 3))
           20)
        "test_let_inside_closure_returns_correct_value")
...
(assert (= (begin
             ...
             (let [r1 (f 10)
               ...
               (+ r1 r2)))
           51)
        "test_multiple_closures_with_let_dont_interfere")
...
(assert (= (begin
             ...
             (let [msg "say hello world"]
               (checker msg)))
           true)
        "test_closure_let_with_string_operations")
