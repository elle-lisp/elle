...
(assert-eq (set 1 2 3) (set 1 2 3) "test")       # immutable set
(assert-eq @|1 2 3| @|1 2 3| "test")             # mutable set
(assert-eq (set 3 1 2) (set 1 2 3) "test")       # order doesn't matter
(assert-eq (set 1 1 2) (set 1 2) "test")         # deduplication

## ── Constructors ────────────────────────────────────────────────────

(assert-eq (set 1 2 3) (set 1 2 3) "test")
(assert-eq (@set 1 2 3) @|1 2 3| "test")
(assert-eq (set) || "test")
(assert-eq (@set) @|| "test")
(assert-eq (set 1 1 2 2 3) (set 1 2 3) "test")   # dedup in constructor

## ── Predicates ──────────────────────────────────────────────────────

(assert-true (set? (set 1 2 3)) "test")
...
(assert-eq (type-of (set 1 2 3)) :set "test")
(assert-eq (type-of @|1 2 3|) :@set "test")

## ── Membership ──────────────────────────────────────────────────────

(assert-true (contains? (set 1 2 3) 2) "test")
(assert-false (contains? (set 1 2 3) 4) "test")
...
(assert-eq (add (set 1 2) 3) (set 1 2 3) "test")
(assert-eq (add (set 1 2) 2) (set 1 2) "test")   # no-op if already present
...
(assert-eq (del (set 1 2 3) 2) (set 1 3) "test")
(assert-eq (del (set 1 2 3) 4) (set 1 2 3) "test") # no-op if not present
...
(assert-eq (union (set 1 2) (set 2 3)) (set 1 2 3) "test")
(assert-eq (intersection (set 1 2 3) (set 2 3 4)) (set 2 3) "test")
(assert-eq (difference (set 1 2 3) (set 2 3)) (set 1) "test")
...
(assert-eq (length (set 1 2 3)) 3 "test")
(assert-eq (length ||) 0 "test")
(assert-eq (length @|1 2|) 2 "test")
(assert-true (empty? ||) "test")
(assert-true (empty? @||) "test")
(assert-false (empty? (set 1)) "test")

## ── Freeze and thaw ────────────────────────────────────────────────

(assert-eq (freeze @|1 2 3|) (set 1 2 3) "test")
(assert-true (set? (freeze @|1 2 3|)) "test")
(assert-eq (type-of (freeze @|1 2 3|)) :set "test")
(assert-eq (thaw (set 1 2 3)) @|1 2 3| "test")
(assert-true (set? (thaw (set 1 2 3))) "test")
(assert-eq (type-of (thaw (set 1 2 3))) :@set "test")

## ── Conversion ──────────────────────────────────────────────────────

(assert-eq (length (set->array (set 3 1 2))) 3 "test")
