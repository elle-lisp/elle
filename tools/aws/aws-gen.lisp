...
(defn https-get [host path]
  ...
  (defer
    ...
    (forever
      ...
      (when (not (nil? colon))
        (put headers (keyword (string/lowercase (slice hline 0 colon)))
...
(each service in services
  ...
  (def needs-gen
    (or (nil? output-stat) (> model-stat:modified output-stat:modified)))

  (if needs-gen
    (begin
      (def result
        (subprocess/system elle-bin ["tools/aws/aws-codegen.lisp" "--" service]))
      (if (= result:exit 0)
        (begin
