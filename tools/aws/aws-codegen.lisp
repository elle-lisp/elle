...
(defn emit-rest-function [op-name op-shape]
  ...
  (emit-line)
  (def param-names (map camel->kebab positional))
  (def param-list (string/join (append param-names (list "&keys opts")) " "))
...
(defn emit-json-function [op-name op-shape]
  ...
  (emit-line)
  (def param-names (map (fn [m] (camel->kebab m:name)) positional))
  (def param-list (string/join (append param-names (list "&keys opts")) " "))
  (indent 1 "(defn " fn-name " [" param-list "]")
