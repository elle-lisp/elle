...
(print (query "SELECT ?type (COUNT(*) AS ?count)
               WHERE { ?s a ?type }
               GROUP BY ?type
               ORDER BY DESC(?count)"))
...
(print (query "SELECT ?file (COUNT(*) AS ?fns)
...
(print (query "SELECT ?name ?path ?file
...
(print (query "SELECT ?name ?arity ?file
...
(print (query "SELECT ?name (GROUP_CONCAT(DISTINCT ?file; separator=\", \") AS ?files) (COUNT(DISTINCT ?file) AS ?n)
...
(print (query "SELECT ?param (COUNT(*) AS ?uses)
