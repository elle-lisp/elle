...
(def result (gpu:run shader [wg-count 1 1]
              [(gpu:input cx) (gpu:input cy) (gpu:output N)]))
...
(def origin-idx (+ (int (/ (* WIDTH (- 0.0 Y-MIN)) (- Y-MAX Y-MIN)))
                   (int (/ (* WIDTH (- 0.0 X-MIN)) (- X-MAX X-MIN)))))
## Approximate center pixel
(def center-idx (+ (* (/ HEIGHT 2) WIDTH) (/ WIDTH 2)))

(println "  Center pixel (" center-idx "): " (int (result center-idx)) "iterations")
