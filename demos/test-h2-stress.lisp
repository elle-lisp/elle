...
(defn test-stream-reader []
  ...
  (let [
        ## Build 100 messages, each ~1200 bytes (total ~120KB > 7 DATA frames)
...
(defn test-abandoned-stream []
  ...
  (let [
        ## 100 messages — more than data-queue capacity (64)
...
(defn test-cohort-pipeline []
  ...
  (let [windows [5 20 60 120 180 250]  ## Each job ≈ window_days × 33 features × 8 bytes (protobuf overhead)
        ## × 10 jobs per layer
  ...
  (let [windows [5 20 60 120 180 250]
    ...
    (with-server (fn [req]
                   (cond
                     ...
                     (= req:path "/stream")
                       (let [
                             ## Simulate Subscribe: return num-jobs gRPC-framed result events
                             events (apply concat
                             (map (fn [i]
                                    (grpc-frame (bytes (bit/shr i 8)
                                    (bit/and i 0xff) 0 0))) (range 0 num-jobs)))]
