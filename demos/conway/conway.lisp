...
(defn set-cell [g r c v] (put g (idx r c) v))
...
(defn step [g]
  ...
  (while (< r (- rows 1))
    ...
    (while (< c (- cols 1))
      ...
      (assign c (+ c 1)))
    (assign r (+ r 1)))
  # Border cells are always dead (simpler than wrapping)
  nxt)
...
(defn main []
  ...
  (def win (sdl:create-window "Conway's Game of Life" win-w win-h
  ...
  (while (state :running)
    ...
    # Render
    (render ren (state :grid) (state :gen)
            alive (state :paused) (state :speed) fps
            (state :show-grid)))
