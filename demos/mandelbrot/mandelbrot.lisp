...
(def [gpu-ok? gpu]
  (when vk-ok?
    (protect ((import "std/gpu") :vulkan vk-plugin))))
(def gpu-ctx
  (when gpu-ok?
...
(defn compute-mandelbrot-gpu []
  ...
  (def handle
    (gpu-plugin:dispatch gpu-shader
      wg-count
      1
      1
...
(defn on-draw [_da cairo-ctx w h _data]
  ...
  (let* [s (min (/ (float w) (float WIDTH)) (/ (float h) (float HEIGHT)))
         ...
         surf (cr:image-surface-for-data pixel-buf
           cr:FORMAT_ARGB32
           WIDTH
           HEIGHT
           STRIDE)]
...
(defn on-key [keyval keycode state]
  (let [step (/ (view :scale) 4.0)]
    (cond
      (or (= keyval 0xff1b) (= keyval 0x71)) (begin
        (gtk:close handle)
        (assign quit? true)
        1)
      (= keyval 0x72) (begin
